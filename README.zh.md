...
对于全新的 Linux 或 macOS 机器，推荐直接使用发布版安装脚本：
...
- 检测当前 OS、架构、可用包管理器、Docker 可用性以及可写安装目录
- 下载匹配的 GitHub Release 压缩包和 `checksums.txt`
- 在安装 `openase` 前校验发布包 checksum
- 在条件满足时提供系统包管理器或 Docker 的 PostgreSQL 引导
- 在自动 setup 成功时写入可直接运行的 `~/.openase/config.yaml` 和 `~/.openase/.env`

如果你想固定某个版本，而不是安装最新版本：

```bash
curl -fsSL https://raw.githubusercontent.com/pacificstudio/openase/main/scripts/install.sh | sh -s -- --version v0.4.0
```

如果你更希望从源码构建，请继续阅读下面的手动步骤。
