...
func newRootProjectCommand() *cobra.Command {
	...
	command.Long = strings.TrimSpace(command.Long +
		"\n\nShared project mutations (`update` and `add-repo`) use the agent-platform wrapper semantics in agent workspaces. The remaining subcommands (`list`, `get`, `create`, `delete`) remain direct OpenAPI operations.")
