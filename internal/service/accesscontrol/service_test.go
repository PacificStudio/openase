...
func TestServiceRecoversStoredSecretFromLegacyConfigWhenCipherSeedChanges(t *testing.T) {
	t.Parallel()
...
`)
	...
	second, err := New(repo.NewEntRepository(client), "new-seed", configPath, "")
	if err != nil {
		t.Fatalf("second New() error = %v", err)
	}
	result, err := second.Read(ctx)
	...
	third, err := New(repo.NewEntRepository(client), "new-seed", "", "")
	if err != nil {
		t.Fatalf("third New() error = %v", err)
	}
	reloaded, err := third.Read(ctx)
	if err != nil {
		t.Fatalf("reloaded Read() error = %v", err)
	}
	if reloaded.State.Active == nil || reloaded.State.Active.ClientSecret != "review-secret" {
