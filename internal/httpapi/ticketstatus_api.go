...
func (s *Server) handleListTicketStatuses(c echo.Context) error {
	...
	statuses, err := service.List(c.Request().Context(), projectID)
	if err != nil {
		return writeTicketStatusError(c, err)
	}

	return c.JSON(http.StatusOK, map[string]any{"statuses": statuses})
}
...
func (s *Server) handleResetTicketStatuses(c echo.Context) error {
	...
	statuses, err := service.ResetToDefaultTemplate(c.Request().Context(), projectID)
	if err != nil {
		return writeTicketStatusError(c, err)
	}
