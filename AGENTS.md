...
- The self-hosted GitHub Actions workspace can retain stale `web/` files between runs when a job uses `actions/checkout` with `clean: false`. If Frontend Checks suddenly fail `prettier --check .` on deleted E2E specs or stray `web/test-results/.last-run.json`, prefer restoring a clean checkout for that job instead of chasing nonexistent formatting drift in the current branch.
