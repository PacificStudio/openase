...
- `scripts/ci/backend_coverage.sh` rewrites `HOME` to a temp directory before `make check`, so pgtest-backed packages lose access to cached embedded Postgres binaries unless `OPENASE_PGTEST_SHARED_ROOT` points at a stable extracted cache. In this workspace use `OPENASE_PGTEST_SHARED_ROOT=/home/yuzhong/.cache/openase/pgtest` so `initdb` can find sibling binaries like `postgres` instead of reusing an incomplete temp-home extraction.
