...
- `go test ./...` may also intermittently fail in embedded-postgres-backed packages with `unable to remove password file '/home/yuzhong/.embedded-postgres-go/extracted/pwfile': ... no such file or directory` when multiple packages race on the shared extracted binaries directory. Re-run the affected package(s) directly to confirm whether it is an environment/cache race before treating it as a code regression.
