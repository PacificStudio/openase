...
- `scripts/ci/backend_coverage.sh` swaps `HOME` to a temp dir before `make check`; when pgtest-backed packages then boot embedded Postgres, set `OPENASE_PGTEST_SHARED_ROOT=/home/yuzhong/.cache/openase/pgtest` (or another stable extracted cache) so `initdb` can find sibling binaries like `postgres` instead of reusing an incomplete temp-home extraction.
