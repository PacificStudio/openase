...
- The self-hosted GitHub Actions workspace can retain stale `web/` files between runs when a job uses `actions/checkout` with `clean: false`. If Frontend Checks suddenly fail `prettier --check .` on deleted E2E specs or stray `web/test-results/.last-run.json`, prefer rebuilding the `web/` subtree from `HEAD` inside `working-directory: web` with `find . -mindepth 1 -maxdepth 1 -exec rm -rf {} +` followed by `git restore --source=HEAD --staged --worktree -- .`; a full-repo `clean: true` checkout can still fail if repo-local `.cache/openase-go/...` contains permission-restricted files from earlier jobs.
...
- `scripts/ci/backend_coverage.sh` rewrites `HOME` to a temp directory before `make check`. In this workspace export `OPENASE_PGTEST_SHARED_ROOT=/home/yuzhong/.cache/openase/pgtest` so `internal/testutil/pgtest` reuses the extracted binaries instead of a cold temp-home cache that may miss the sibling `postgres` binary next to `initdb`.
