...
- `scripts/ci/backend_coverage.sh` rewrites `HOME` to a temp directory, so embedded-postgres tests lose access to cached pgtest binaries unless `OPENASE_PGTEST_SHARED_ROOT` points at the real cache root. In this workspace use `OPENASE_PGTEST_SHARED_ROOT=/home/yuzhong/.cache/openase/pgtest` when running `make check` locally after the binaries have already been downloaded there.
