<script lang="ts">
  ...
  function renderFrontmatter(content: string) {
    ...
    rendered = rendered.replace(/("[^"]*"|'[^']*')/g, '<span class="text-emerald-300">$1</span>')
    rendered = rendered.replace(
  ...
  function renderBody(content: string) {
    let rendered = escapeHTML(content)
    rendered = rendered.replace(/\{\{[\s\S]*?\}\}/g, '<span class="text-sky-300">$&</span>')
    rendered = rendered.replace(/\{%[\s\S]*?%\}/g, '<span class="text-emerald-300">$&</span>')
    rendered = rendered.replace(/\{#[\s\S]*?#\}/g, '<span class="text-slate-500 italic">$&</span>')
    return rendered
  }
...
>
  <div class="flex items-center justify-between gap-4 border-b border-slate-800/90 px-5 py-3">
    <div
      class="flex items-center gap-2 text-[11px] font-medium tracking-[0.32em] text-slate-400 uppercase"
    >
