<script lang="ts">
  ...
  const workspaceStats = $derived({
    runningAgents: Object.values(orgMetrics).reduce(
      (sum, metrics) => sum + metrics.runningAgents,
      0,
    ),
    activeTickets: Object.values(orgMetrics).reduce(
      (sum, metrics) => sum + metrics.activeTickets,
      0,
    ),
    todayCost: Object.values(orgMetrics).reduce((sum, metrics) => sum + metrics.todayCost, 0),
    totalTokens: 0,
  })

  const totalProjects = $derived(
    Object.values(orgMetrics).reduce((sum, metrics) => sum + metrics.projectCount, 0),
  )
