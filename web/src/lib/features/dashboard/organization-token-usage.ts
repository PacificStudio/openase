import { getOrganizationTokenUsage } from '$lib/api/openase'
import type { OrganizationTokenUsageDay, OrganizationTokenUsageResponse } from '$lib/api/contracts'
import type {
  OrganizationTokenUsageAnalytics,
  OrganizationTokenUsageDayPoint,
  OrganizationTokenUsagePeak,
  OrganizationTokenUsageRange,
} from './types'

export const organizationTokenUsageRangeOptions = [
  7, 30, 90, 365,
] as const satisfies readonly OrganizationTokenUsageRange[]
...
export function emptyOrganizationTokenUsageAnalytics(
...
export function buildOrganizationTokenUsageQuery(
  rangeDays: OrganizationTokenUsageRange,
  now = new Date(),
) {
...
export function mapOrganizationTokenUsage(
  payload: OrganizationTokenUsageResponse,
  rangeDays: OrganizationTokenUsageRange,
  now = new Date(),
): OrganizationTokenUsageAnalytics {
  const query = buildOrganizationTokenUsageQuery(rangeDays, now)
  const rawByDate = new Map(
    (payload.days ?? [])
      .filter((day): day is OrganizationTokenUsageDay & { date: string } => Boolean(day.date))
      .map((day) => [day.date, day] as const),
  )

  const days: OrganizationTokenUsageDayPoint[] = []
...
export async function loadOrganizationTokenUsage(
  ...
  const query = buildOrganizationTokenUsageQuery(rangeDays, opts?.now)
  const payload = await getOrganizationTokenUsage(orgId, query, opts)
...
function mapPeakDay(
...
): OrganizationTokenUsagePeak | null {
  ...
  const computedPeakDay = days.reduce<OrganizationTokenUsageDayPoint | null>((current, day) => {
...
function buildCalendarCells(days: OrganizationTokenUsageDayPoint[]) {
  if (days.length === 0) return []
...
): OrganizationTokenUsageDayPoint['intensity'] {
