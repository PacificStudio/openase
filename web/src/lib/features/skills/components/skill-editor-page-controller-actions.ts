...
function currentEntrypointContent(state: SkillEditorPageControllerActionsState) {
  return state.getDraftFiles().find((file) => file.path === 'SKILL.md')?.content ?? ''
}

function buildBundleRequestFiles(state: SkillEditorPageControllerActionsState) {
