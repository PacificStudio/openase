...
describe('ProjectConversationWorkspaceBrowser', () => {
  ...
  it('loads and renders file previews when a file is selected from the explorer', async () => {
    ...
    expect(view.getByTestId('workspace-browser-detail-panel').className).toContain('overflow-hidden')
    expect(view.getByTestId('workspace-browser-detail-content').className).toContain('overflow-hidden')
    expect(view.getByTestId('workspace-browser-detail-scroll-frame').className).toContain('overflow-hidden')
