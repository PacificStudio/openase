<script lang="ts">
  ...
  async function handleGraphCheckout(request: {
    ...
    try {
      ...
      graphActionError =
        error instanceof ApiError ? error.detail : error instanceof Error ? error.message : 'Failed to switch branches.'
    }
  }
  ...
  const mode = $derived(browser.detailMode)
...
<div class="flex h-full min-h-0 flex-col overflow-hidden">
  ...
  {:else}
    <div class="border-border bg-muted/20 flex items-center gap-1.5 border-b px-3 py-1.5">
      <Button
        size="sm"
        variant={mode === 'file' ? 'secondary' : 'ghost'}
        onclick={() => browser.setDetailMode('file')}
      >
        Files
      </Button>
      <Button
        size="sm"
        variant={mode === 'git_graph' ? 'secondary' : 'ghost'}
        onclick={() => browser.setDetailMode('git_graph')}
      >
        Git Graph
      </Button>
    </div>
