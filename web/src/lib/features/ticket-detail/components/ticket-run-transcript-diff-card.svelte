<script lang="ts">
  ...
  let { block }: { block: Extract<TicketRunTranscriptBlock, { kind: 'diff' }> } = $props()

  let expanded = $state(false)

  const adds = $derived(
    block.diff.hunks.reduce((n, h) => n + h.lines.filter((l) => l.op === 'add').length, 0),
  )
  const removes = $derived(
    block.diff.hunks.reduce((n, h) => n + h.lines.filter((l) => l.op === 'remove').length, 0),
  )
...
<div class="group">
  ...
  >
    ...
    <span class="text-foreground min-w-0 flex-1 truncate font-mono">
      {shortenPath(block.diff.file)}
    </span>
    <span class="text-muted-foreground/60 flex shrink-0 items-center gap-1.5 text-[10px]">
      ...
      <span>{block.diff.hunks.length} hunk{block.diff.hunks.length === 1 ? '' : 's'}</span>
    </span>
  </button>
  ...
  {#if expanded}
    <div class="border-border/40 ml-5 border-l pt-1 pb-2 pl-3">
      <div class="overflow-hidden rounded-md bg-slate-950">
        {#each block.diff.hunks as hunk, i}
          <div
