<script lang="ts">
  ...
  const columnLabelKeys: Record<
    'status' | 'run' | 'agent' | 'workflow' | 'ticket' | 'lastHeartbeat' | 'actions',
    TranslationKey
  > = {
    status: 'agents.agentRunList.column.status',
    run: 'agents.agentRunList.column.run',
    agent: 'agents.agentRunList.column.agent',
    workflow: 'agents.agentRunList.column.workflow',
    ticket: 'agents.agentRunList.column.ticket',
    lastHeartbeat: 'agents.agentRunList.column.lastHeartbeat',
    actions: 'agents.agentRunList.column.actions',
  }

  const statusLabelKeys: Record<AgentRunInstance['status'], TranslationKey> = {
    launching: 'agents.agentRunStatus.launching',
    ready: 'agents.agentRunStatus.ready',
    executing: 'agents.agentRunStatus.executing',
    completed: 'agents.agentRunStatus.completed',
    errored: 'agents.agentRunStatus.errored',
    interrupted: 'agents.agentRunStatus.interrupted',
    terminated: 'agents.agentRunStatus.terminated',
  }
  ...
  <table class="w-full text-sm">
    <thead>
      <tr class="border-border text-muted-foreground border-b text-left text-xs">
        ...
        <th class="px-2 pb-2 font-medium">{i18nStore.t(columnLabelKeys.run)}</th>
      ...
      {#if agentRuns.length === 0}
        <tr>
          <td colspan="7" class="text-muted-foreground px-3 py-8 text-center text-sm">
            {i18nStore.t('agents.agentRunList.emptyState')}
          </td>
        </tr>
      {:else}
          ...
          <tr class="group border-border/50 hover:bg-muted/30 border-b transition-colors">
            ...
            <td class="px-2 py-2.5">
              <div class="text-foreground font-medium">
                {i18nStore.t('agents.agentRunList.runLabel', { id: shortRunId(agentRun.id) })}
              </div>
            ...
            <td class="px-2 py-2.5">
              <div class="text-muted-foreground text-xs">
                ...
                {:else if agentRun.runtimeStartedAt}
                  {i18nStore.t('agents.agentRunList.startedMessage', {
                    relativeTime: formatRelativeTime(agentRun.runtimeStartedAt),
                  })}
                {:else}
                  &mdash;
                {/if}
            ...
            <td class="py-2.5 pr-3 pl-2">
              ...
              >
                <Button
                  variant="ghost"
                  size="icon-xs"
                  aria-label={i18nStore.t('agents.agentRunList.action.viewOutput')}
                  title={i18nStore.t('agents.agentRunList.action.viewRuntimeOutput')}
