    ...
    return {
      ...
      description: complete
        ...
        : (currentMilestone?.description ??
          'OpenASE is still unlocking core onboarding milestones.'),
...
<div class="relative overflow-hidden">
  <div
    class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(194,120,3,0.2),transparent_28rem),radial-gradient(circle_at_bottom_right,rgba(13,148,136,0.18),transparent_30rem)]"
  ></div>

  <section
    class="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-8 px-6 py-8 sm:px-8 lg:px-10"
  >
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
      <div class="space-y-5">
        ...
        <div class="space-y-4">
          <p class="text-muted-foreground text-sm font-medium tracking-[0.32em] uppercase">
            OpenASE control plane
          </p>
          <h1
            class="max-w-4xl text-5xl leading-none font-semibold tracking-[-0.06em] text-balance sm:text-6xl"
          >
            Kanban routing, live agent telemetry, and workflow harness editing now share one control
            plane.
          </h1>
          <p class="text-muted-foreground max-w-3xl text-lg leading-8">
    ...
    {#if errorMessage}
      <div
        class="border-destructive/25 bg-destructive/10 text-destructive rounded-3xl border px-5 py-4 text-sm"
      >
        {errorMessage}
      </div>
    {/if}
    ...
    <div
      class="overflow-hidden rounded-[2rem] border border-amber-500/20 bg-[linear-gradient(135deg,rgba(245,158,11,0.18),rgba(255,255,255,0.92)_42%,rgba(16,185,129,0.12))] shadow-[0_24px_80px_-48px_rgba(120,53,15,0.65)]"
    >
      <div class="grid gap-6 p-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(20rem,0.85fr)] lg:p-8">
        <div class="space-y-5">
          ...
          <div class="space-y-3">
            <p class="text-sm font-medium tracking-[0.28em] text-amber-950/70 uppercase">
              Chapter 14 onboarding
            </p>
            <h2
              class="max-w-3xl text-3xl leading-tight font-semibold tracking-[-0.05em] text-balance text-slate-950 sm:text-4xl"
            >
          ...
          {#each onboardingSummary.milestones as milestone}
            <div class={`rounded-3xl border px-4 py-4 ${onboardingCardClass(milestone)}`}>
              <div class="flex items-start justify-between gap-3">
                ...
                <span
                  class={`inline-flex shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-medium ${onboardingStatusClass(milestone)}`}
                >
                  {#if milestone.completed}
                    Done
              ...
              <p class="mt-3 text-xs font-medium tracking-[0.2em] uppercase opacity-70">
                {milestone.action}
              </p>
            </div>
          {/each}
      ...
      <div class="grid gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]">
          ...
          <Card class="border-border/80 bg-background/80 backdrop-blur">
            <CardHeader>
              <div class="flex items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription
                    >Pick the workspace boundary before creating projects.</CardDescription
                  >
                </div>
            ...
            <CardContent class="space-y-3">
              {#if organizations.length === 0}
                <div
                  class="border-border/80 bg-muted/35 text-muted-foreground rounded-3xl border border-dashed px-4 py-6 text-sm"
                >
        ...
        <div class="space-y-6">
          <Card class="border-border/80 bg-background/80 overflow-hidden backdrop-blur">
            <CardHeader class="border-border/70 bg-muted/25 border-b">
            ...
            <div class="space-y-6">
              <Card class="border-border/80 bg-background/80 backdrop-blur">
                <CardHeader>
                  ...
                  <CardDescription>
                    A minimal project record carries routing metadata and runtime concurrency
                    limits.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div class="space-y-2">
                          ...
                          <input
                            ...
                            class={inputClass}
                            bind:value={editProjectForm.slug}
                          />
                        </div>
                      </div>
                      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_12rem]">
          ...
          <Card class="border-border/80 bg-background/80 overflow-hidden backdrop-blur">
            ...
            <CardContent class="space-y-5 p-6">
              ...
              {:else if ticketBoardBusy && tickets.length === 0}
                <div
                  class="border-border/70 bg-background/60 flex min-h-72 items-center justify-center rounded-[2rem] border"
                >
                  <div class="text-muted-foreground flex items-center gap-3 text-sm">
                    <LoaderCircle class="size-4 animate-spin" />
              ...
              {:else}
                <div
                  class="border-border/70 bg-muted/25 text-muted-foreground flex flex-wrap items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-sm"
                >
                  <p>
                    <span class="text-foreground font-medium">{selectedProject.name}</span>
                      ...
                      >
                        ...
                        <div class="mt-4 space-y-3">
                          {#if ticketsForStatus(status.id).length === 0}
                            <div
                              class="border-border/70 bg-muted/35 text-muted-foreground rounded-3xl border border-dashed px-4 py-6 text-sm"
                            >
                            ...
                            {#each ticketsForStatus(status.id) as ticket}
                              ...
                              >
                                <div class="flex items-start justify-between gap-3">
                                  <div class="min-w-0">
                                    <p
                                      class="text-muted-foreground font-mono text-[11px] tracking-[0.2em] uppercase"
                                    >
                                      {ticket.identifier}
                                    </p>
                                  ...
                                  <span
                                    class={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-medium ${ticketPriorityBadgeClass(ticket.priority)}`}
                                  >
                                    {ticket.priority}
                                  </span>
                                </div>
                                ...
                                <div class="mt-4 flex flex-wrap gap-2 text-[11px]">
                                  <span
                                    class="border-border/80 bg-background text-muted-foreground inline-flex rounded-full border px-2.5 py-1"
                                  >
                                    {ticket.type}
                                  </span>
                                  ...
                                  {#if ticket.consecutive_errors > 0}
                                    <span
                                      class="inline-flex rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-1 text-amber-700"
                                    >
                                  ...
                                  {#if ticket.retry_paused}
                                    <span
                                      class="inline-flex rounded-full border border-rose-500/25 bg-rose-500/10 px-2.5 py-1 text-rose-700"
                                    >
                                ...
                                <div
                                  class="text-muted-foreground mt-4 flex items-center justify-between gap-3 text-xs"
                                >
                                  <span>{formatTimestamp(ticket.created_at)}</span>
                                  <div class="flex items-center gap-3">
                                    <a
                                      href={ticketDetailHref(ticket.id)}
                                      class="border-border/70 bg-background text-foreground hover:border-foreground/20 inline-flex items-center gap-1 rounded-full border px-2.5 py-1 font-medium transition"
            ...
            <CardHeader class="border-border/70 bg-muted/25 border-b">
              <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    Status snapshot, realtime stream state, and heartbeat aging for the selected
                    project.
                  </CardDescription>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                  <span
                    class={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium ${streamBadgeClass(agentStreamState)}`}
                  >
                    <Cable class="mr-1.5 size-3.5" />
                ...
                <div class="grid gap-4 md:grid-cols-3">
                  ...
                  <Card class="border-border/70 bg-background/70">
                    <CardHeader class="pb-3">
                      ...
                      <CardTitle class="text-3xl tracking-[-0.05em]"
                        >{runningAgentCount()}</CardTitle
                      >
                    </CardHeader>
                  </Card>
                  <Card class="border-border/70 bg-background/70">
                          ...
                          >
                            <div class="flex items-start justify-between gap-4">
                              <div>
                                ...
                                <p
                                  class={`mt-1 text-xs ${agent.id === selectedAgentId ? 'text-white/70' : 'text-muted-foreground'}`}
                                >
                              ...
                              <span
                                class={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-medium ${
                                  agent.id === selectedAgentId
                                    ...
                                    : heartbeatBadgeClass(agent.last_heartbeat_at)
                                }`}
                              >
                            ...
                            <p
                              class={`mt-3 text-sm ${agent.id === selectedAgentId ? 'text-white/80' : 'text-muted-foreground'}`}
                            >
                    ...
                    <Card class="border-border/70 bg-background/70">
                      ...
                      <CardContent>
                        {#if currentAgent()}
                          <div class="grid gap-4 md:grid-cols-2">
                            <div
                              class="border-border/70 bg-background rounded-3xl border px-4 py-4"
                            >
                              <p
                                class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                              >
                              ...
                              <p class="text-muted-foreground mt-2 text-sm">
                                Last seen at {formatTimestamp(
                                  currentAgent()?.last_heartbeat_at ?? '',
                                )}
                              </p>
                            </div>
                            <div
                            ...
                            >
                              ...
                              <p class="text-muted-foreground mt-2 text-sm">
                                Tokens {currentAgent()?.total_tokens_used ?? 0} · Completed {currentAgent()
                                  ?.total_tickets_completed ?? 0}
                              </p>
                            </div>
                            <div
                              class="border-border/70 bg-background rounded-3xl border px-4 py-4 md:col-span-2"
                            >
                              <p
                                class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                              >
                      ...
                      <CardContent>
                        {#if activityEvents.length === 0}
                          <div
                            class="border-border/80 bg-muted/35 text-muted-foreground rounded-3xl border border-dashed px-4 py-6 text-sm"
                          >
                            No activity lines yet. The console will append new events as the backend
                            publishes into `activity.events`.
                          </div>
                        {:else}
                          <div class="max-h-[32rem] space-y-3 overflow-y-auto pr-1">
                            {#each activityEvents as item}
                              ...
                              >
                                ...
                                <p
                                  class="text-foreground/90 mt-3 text-sm leading-6 whitespace-pre-wrap"
                                >
                                  {item.message || 'No message payload'}
                                </p>
                                {#if Object.keys(item.metadata).length > 0}
                                  <pre
                                    class="bg-muted/60 text-muted-foreground mt-3 overflow-x-auto rounded-2xl px-3 py-3 text-xs">{JSON.stringify(
                                      item.metadata,
                                      null,
                                      2,
                                    )}</pre>
          ...
          <Card class="border-border/80 bg-background/80 overflow-hidden backdrop-blur">
            <CardHeader class="border-border/70 bg-muted/20 border-b">
              <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription>
                    Manage workflow metadata and the Git-backed harness document for the selected
                    project.
                  </CardDescription>
                </div>
                {#if selectedProject}
            ...
            <CardContent class="p-6">
              {#if !selectedProject}
                <div
                  class="border-border/80 bg-muted/35 text-muted-foreground rounded-3xl border border-dashed px-4 py-10 text-sm"
                >
                        ...
                        {:else}
                          {#each workflows as workflow}
                            ...
                            >
                              ...
                              >
                                {statusName(workflow.pickup_status_id)} -> {statusName(
                                  workflow.finish_status_id,
                                )}
                              </p>
                            </button>
                          {/each}
                      ...
                      <CardHeader>
                        <div class="flex items-center justify-between gap-4">
                          <div>
                            ...
                            <CardDescription>
                              Start from one of the built-in Harness templates, then tune the
                          ...
                          >
                            <div class="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <p class="text-muted-foreground mt-2 text-sm">{role.summary}</p>
                              </div>
                              <Badge
                                variant={role.slug === selectedBuiltinRoleSlug
                                  ? 'secondary'
                                  : 'outline'}
                              >
                                {role.workflow_type}
                      ...
                      <CardContent>
                        ...
                        >
                          {#if selectedBuiltinRole}
                            <div
                              class="rounded-2xl border border-emerald-500/25 bg-emerald-500/10 px-4 py-4 text-sm text-emerald-950"
                            >
                              <div class="flex flex-wrap items-center justify-between gap-3">
                                <div>
                                  <p class="font-semibold">{selectedBuiltinRole.name}</p>
                                  <p class="mt-1 text-emerald-900/80">
                                    {selectedBuiltinRole.summary}
                                  </p>
                                </div>
                                <Button
                                  type="button"
                                  variant="outline"
                                  onclick={() => clearBuiltinRoleSelection()}
                                >
                                  Clear template
                                </Button>
                              </div>
                          ...
                          <Button
                            class="w-full"
                            type="submit"
                            disabled={workflowBusy || ticketStatuses.length === 0}
                          >
                            {#if workflowBusy}
                              <LoaderCircle class="mr-2 size-4 animate-spin" />
                            {:else}
                      ...
                      <CardHeader>
                        <div class="flex items-center justify-between gap-4">
                          <div>
                            ...
                            <CardDescription>
                              Inspect built-in and harvested skills, then bind the selected workflow
                      ...
                      <CardContent class="space-y-3">
                        ...
                        {:else}
                          {#each skills as skill}
                            <div
                              class="border-border/70 bg-background/60 rounded-3xl border px-4 py-4"
                            >
                              <div class="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                  ...
                                  <p class="text-muted-foreground mt-2 text-sm">
                                    {skill.description ||
                                      'No description extracted from SKILL.md yet.'}
                                  </p>
                                  <p class="text-muted-foreground mt-2 font-mono text-[11px]">
                                    {skill.path}
                                  </p>
                                </div>
                                {#if selectedWorkflow}
                                  <Button
                                    type="button"
                                    variant={workflowHasSkill(skill, selectedWorkflow.id)
                                      ? 'outline'
                                      : 'default'}
                                  ...
                                  >
                                    {#if skillBusy && pendingSkillName === skill.name}
                                      <LoaderCircle class="mr-2 size-4 animate-spin" />
                                    {/if}
                                    {workflowHasSkill(skill, selectedWorkflow.id)
                                      ? 'Unbind'
                                      : 'Bind'}
                                  </Button>
                                {/if}
                        ...
                        {#if !selectedWorkflow}
                          <div
                            class="border-border/80 bg-muted/35 text-muted-foreground rounded-2xl border border-dashed px-4 py-4 text-sm"
                          >
                        ...
                        {:else if harnessDirty}
                          ...
                          >
                            Save the current harness draft before changing skill bindings, so the
                            frontmatter update does not overwrite unsaved edits.
                          </div>
                        {/if}
                      </CardContent>
                    </Card>
                          ...
                          >
                            <div class="grid gap-4 lg:grid-cols-2">
                              <div class="space-y-2">
                                ...
                                <input
                                  ...
                                  class={inputClass}
                                  bind:value={editWorkflowForm.name}
                                />
                              </div>
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-type"
                                >
                                  Type
                                </label>
                                <select
                                  ...
                                  class={inputClass}
                                  bind:value={editWorkflowForm.type}
                                >
                                  {#each workflowTypes as workflowType}
                                    <option value={workflowType}>{workflowType}</option>
                                  {/each}
                            ...
                            <div class="grid gap-4 lg:grid-cols-2">
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-pickup-status"
                                >
                                  Pickup status
                                </label>
                                <select
                                  id="edit-workflow-pickup-status"
                                  class={inputClass}
                                  bind:value={editWorkflowForm.pickupStatusId}
                                >
                                  {#each ticketStatuses as status}
                                    <option value={status.id}>{status.name}</option>
                                  {/each}
                              ...
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-finish-status"
                                >
                                  Finish status
                                </label>
                                <select
                                  id="edit-workflow-finish-status"
                                  class={inputClass}
                                  bind:value={editWorkflowForm.finishStatusId}
                                >
                                  <option value="">No auto-finish status</option>
                                  {#each ticketStatuses as status}
                                    <option value={status.id}>{status.name}</option>
                            ...
                            <div class="grid gap-4 lg:grid-cols-4">
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-max-concurrent"
                                >
                                  Max concurrent
                                </label>
                                <input
                                  id="edit-workflow-max-concurrent"
                                  class={inputClass}
                                  bind:value={editWorkflowForm.maxConcurrent}
                                  type="number"
                                  min="1"
                                />
                              </div>
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-max-retries"
                                >
                                  Max retries
                                </label>
                                <input
                                  id="edit-workflow-max-retries"
                                  class={inputClass}
                                  bind:value={editWorkflowForm.maxRetryAttempts}
                                  type="number"
                                  min="0"
                                />
                              </div>
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-timeout"
                                >
                                  Timeout min
                                </label>
                                <input
                                  id="edit-workflow-timeout"
                                  class={inputClass}
                                  bind:value={editWorkflowForm.timeoutMinutes}
                                  type="number"
                                  min="1"
                                />
                              </div>
                              <div class="space-y-2">
                                <label
                                  class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                                  for="edit-workflow-stall-timeout"
                                >
                                  Stall min
                                </label>
                                <input
                                  id="edit-workflow-stall-timeout"
                                  class={inputClass}
                                  bind:value={editWorkflowForm.stallTimeoutMinutes}
                                  type="number"
                                  min="1"
                                />
                              </div>
                            ...
                            >
                              <input
                                bind:checked={editWorkflowForm.isActive}
                                class="border-border size-4 rounded"
                                type="checkbox"
                              />
                              <span>Workflow is active and dispatchable</span>
                            </label>
                            <div class="border-border/70 bg-muted/30 rounded-2xl border px-4 py-3">
                              <p
                                class="text-muted-foreground text-xs font-medium tracking-[0.24em] uppercase"
                              >
                                Harness path
                              </p>
                              <p class="text-foreground/80 mt-2 font-mono text-xs">{harnessPath}</p>
                            </div>
                            <div class="flex flex-wrap gap-3">
                      ...
                      <CardHeader>
                        <div class="flex flex-wrap items-center justify-between gap-4">
                          <div>
                            ...
                            <CardDescription>
                              YAML frontmatter is validated live. Save writes directly through the
                              workflow API.
                            </CardDescription>
                          </div>
                          <div class="flex flex-wrap items-center gap-2">
                      ...
                      <CardContent class="space-y-4">
                        {#if selectedWorkflow}
                          ...
                          >
                            ...
                            <div class="flex flex-wrap gap-2">
                              <Button
                                ...
                                onclick={() => void validateHarnessNow()}
                              >
                                {#if validationBusy}
                                  <LoaderCircle class="mr-2 size-4 animate-spin" />
                                {:else}
                          ...
                          <div class="grid gap-3">
                            {#if validationBusy}
                              <div
                                class="border-border/70 bg-background/60 text-muted-foreground rounded-2xl border px-4 py-3 text-sm"
                              >
                                <div class="flex items-center gap-2">
                                  <LoaderCircle class="size-4 animate-spin" />
                            ...
                            {:else if harnessIssues.length === 0}
                              <div
                                class="rounded-2xl border border-emerald-500/25 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-900"
                              >
                            ...
                            {:else}
                              {#each harnessIssues as issue}
                                ...
                                >
                                  <p class="font-medium tracking-[0.18em] uppercase">
                                    {issue.level}
                                    {#if issue.line}
                                      {' '}line {issue.line}
                                      {#if issue.column}
