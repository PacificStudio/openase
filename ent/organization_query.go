...
func (_q *OrganizationQuery) QueryProviders() *AgentProviderQuery {
...
// QueryMachines chains the current query on the "machines" edge.
func (_q *OrganizationQuery) QueryMachines() *MachineQuery {
...
func (_q *OrganizationQuery) QueryDefaultAgentProvider() *AgentProviderQuery {
	query := (&AgentProviderClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *OrganizationQuery) WithProviders(opts ...func(*AgentProviderQuery)) *OrganizationQuery {
	query := (&AgentProviderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithMachines tells the query-builder to eager-load the nodes that are connected to
// the "machines" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *OrganizationQuery) WithMachines(opts ...func(*MachineQuery)) *OrganizationQuery {
	query := (&MachineClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMachines = query
	return _q
}
...
func (_q *OrganizationQuery) WithDefaultAgentProvider(opts ...func(*AgentProviderQuery)) *OrganizationQuery {
	query := (&AgentProviderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withMachines; query != nil {
		if err := _q.loadMachines(ctx, query, nodes,
			func(n *Organization) { n.Edges.Machines = []*Machine{} },
			func(n *Organization, e *Machine) { n.Edges.Machines = append(n.Edges.Machines, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withDefaultAgentProvider; query != nil {
		if err := _q.loadDefaultAgentProvider(ctx, query, nodes, nil,
			func(n *Organization, e *AgentProvider) { n.Edges.DefaultAgentProvider = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrganizationQuery) loadProviders(ctx context.Context, query *AgentProviderQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *AgentProvider)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(agentprovider.FieldOrganizationID)
	}
...
func (_q *OrganizationQuery) loadMachines(ctx context.Context, query *MachineQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Machine)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(machine.FieldOrganizationID)
	}
	query.Where(predicate.Machine(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.MachinesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) loadDefaultAgentProvider(ctx context.Context, query *AgentProviderQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *AgentProvider)) error {
...
func (_q *OrganizationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withDefaultAgentProvider != nil {
			_spec.Node.AddColumnOnce(organization.FieldDefaultAgentProviderID)
		}
	}
