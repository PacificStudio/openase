...
type SecretQuery struct {
	...
	withBindings *SecretBindingQuery
...
func (_q *SecretQuery) QueryBindings() *SecretBindingQuery {
	query := (&SecretBindingClient{config: _q.config}).Query()
...
func (_q *SecretQuery) WithBindings(opts ...func(*SecretBindingQuery)) *SecretQuery {
	query := (&SecretBindingClient{config: _q.config}).Query()
...
//
//	client.Secret.Query().
//		GroupBy(secret.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Secret.Query().
//		Select(secret.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (_q *SecretQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Secret, error) {
	...
	if query := _q.withBindings; query != nil {
		if err := _q.loadBindings(ctx, query, nodes,
			func(n *Secret) { n.Edges.Bindings = []*SecretBinding{} },
			func(n *Secret, e *SecretBinding) { n.Edges.Bindings = append(n.Edges.Bindings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SecretQuery) loadBindings(ctx context.Context, query *SecretBindingQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *SecretBinding)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(secretbinding.FieldSecretID)
	}
	query.Where(predicate.SecretBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(secret.BindingsColumn), fks...))
	}))
