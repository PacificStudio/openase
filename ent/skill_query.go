...
type SkillQuery struct {
	...
	withCurrentVersion   *SkillVersionQuery
	withVersions         *SkillVersionQuery
	withWorkflowBindings *WorkflowSkillBindingQuery
...
func (_q *SkillQuery) QueryCurrentVersion() *SkillVersionQuery {
...
// QueryWorkflowBindings chains the current query on the "workflow_bindings" edge.
func (_q *SkillQuery) QueryWorkflowBindings() *WorkflowSkillBindingQuery {
...
func (_q *SkillQuery) Clone() *SkillQuery {
	...
	return &SkillQuery{
		...
		withWorkflowBindings: _q.withWorkflowBindings.Clone(),
...
func (_q *SkillQuery) WithCurrentVersion(opts ...func(*SkillVersionQuery)) *SkillQuery {
	query := (&SkillVersionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithWorkflowBindings tells the query-builder to eager-load the nodes that are connected to
// the "workflow_bindings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SkillQuery) WithWorkflowBindings(opts ...func(*WorkflowSkillBindingQuery)) *SkillQuery {
	query := (&WorkflowSkillBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withWorkflowBindings = query
	return _q
}
...
func (_q *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := _q.withCurrentVersion; query != nil {
		if err := _q.loadCurrentVersion(ctx, query, nodes, nil,
			func(n *Skill, e *SkillVersion) { n.Edges.CurrentVersion = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withWorkflowBindings; query != nil {
		if err := _q.loadWorkflowBindings(ctx, query, nodes,
			func(n *Skill) { n.Edges.WorkflowBindings = []*WorkflowSkillBinding{} },
			func(n *Skill, e *WorkflowSkillBinding) {
				n.Edges.WorkflowBindings = append(n.Edges.WorkflowBindings, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *SkillQuery) loadCurrentVersion(ctx context.Context, query *SkillVersionQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *SkillVersion)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Skill)
	for i := range nodes {
...
func (_q *SkillQuery) loadWorkflowBindings(ctx context.Context, query *WorkflowSkillBindingQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *WorkflowSkillBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Skill)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workflowskillbinding.FieldSkillID)
	}
	query.Where(predicate.WorkflowSkillBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(skill.WorkflowBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SkillQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCurrentVersion != nil {
			_spec.Node.AddColumnOnce(skill.FieldCurrentVersionID)
		}
	}
