...
- 工作台评论是唯一事实来源：计划、验证、阻塞、推送记录和必要备注都持续写到这一条里。
...
- `In Progress` -> 直接在 `main` 实现、验证并推送；完成后直接进入 `Done`。
- `Rework` -> 继续在 `main` 上实现修正；处理完后重新验证并推送，随后进入 `Done`。
- `Done` -> 终态，不做任何操作。
...
9. 推送完成后更新工作台评论，并将工单直接推进到 `Done`，并关闭关联的工单，避免阻塞后续的任务。

## 额外说明

- 当前阶段不设置 `In Review`，也不等待单独的 review / merge 阶段。
- 如果工单历史上已经存在明确的 issue 评论或 commit 评论需要处理，直接在当前执行轮次里完成修正并推送，不额外停留在 review 状态。
