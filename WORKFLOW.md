...
- `In Progress` -> 直接实现、验证、推送分支并创建或更新 PR；但在进入 `In Review` 前，必须先确认 PR 的 base branch，并把当前分支更新到该 base branch 的最新提交。
...
- `Rework` -> 基于 review 反馈继续实现；回到 `In Review` 前，同样必须把当前分支同步到最新的 PR base branch，并重跑受影响验证。
...
9. 对于 `In Progress` / `Rework` 路径，完成上述 base branch 同步后，更新工作台评论，提交并推送分支。
10. 对于 `In Progress` / `Rework` 路径，如果当前不存在 PR，则创建 PR；如果已存在 PR，则更新其描述、评论和关联信息。
11. 对于 `In Progress` / `Rework` 路径，将工单推进到 `In Review`，并在工作台评论中记录：
   - 分支名
   - 提交 SHA
   - PR 链接
   ...
   - 验证结果
12. 如果当前状态是 `In Review`，则不要继续堆实现，而是直接审核现有 PR / 分支：
...
13. 当工单进入 `Merging`，先同步最新 `origin/main`，处理冲突并重跑受影响验证，再执行合并或仓库规定的落地主干动作。
14. 合并完成后更新工作台评论，记录最终提交与落地结果，并将工单推进到 `Done`。
