...
5. 最终目标是推动工单进入可评审、可合并的状态，而不是输出长篇解释。
...
- 工作台评论是唯一事实来源：计划、验证、阻塞、PR、反馈处理都持续写到这一条里。
...
- `In Progress` -> 直接实现、验证、提交 PR；完成后进入 `In Review`。
- `In Review` -> 等待人类 review 或新的反馈，不主动继续大改。
- `Rework` -> 基于反馈继续实现，处理完后回到 `In Review`。
- `Merging` -> 已批准，可以整理分支、确认最新主干后完成合并动作。
- `Done` -> 终态，不做任何操作。
...
5. 动手前先同步最新主干：
   - 检查当前分支是否已有打开中的 PR
   - 必要时从最新 `origin/main` 创建干净分支
6. 直接实现最小可交付版本，保持改动聚焦在当前工单。
7. 完成后运行必要验证，更新工作台评论，并把工单移到 `In Review`。
8. 如果收到了 review 反馈，逐条处理后回到步骤 6。
9. 当工单进入 `Merging`，确认分支可安全落地后执行合并。
...
- 缺少 GitHub 权限或鉴权，无法更新 PR / 评论 / Project 状态
...
- 若当前工作已无法继续推进，则将工单移动到 `In Review`

## 输出要求

- 回复只报告已完成动作、验证结果、PR/评论链接和阻塞项。
- 不要给人类布置泛泛的“下一步”。
