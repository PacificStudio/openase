...
server:
  port: 40023
workspace:
  root: "${HOME}/agent-workspace/symphony/workspaces"
hooks:
  after_create: |
    git clone --depth 1 git@github.com:PacificStudio/openase.git .
agent:
...
- 先确认当前工单状态，再选择对应流程。
- 每次开工先查找或创建一条持久化工作台评论，标题标记为 `## Workpad`。
...
- 在 `## Workpad` 中写清楚缺什么、为什么阻塞、需要什么人类动作
