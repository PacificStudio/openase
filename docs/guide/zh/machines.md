...
8. 如果已经保留 SSH helper 访问，可以直接使用 `openase machine ssh-bootstrap <machine-id>` 自动安装与拓扑匹配的服务：`reverse_connect` 安装反向 daemon，`direct_connect` 安装远端 listener。
9. 只有在需要 `openase machine ssh-bootstrap` 或 `openase machine ssh-diagnostics` 时才填写 SSH helper 凭证。
10. 点击 `Connection test` 验证当前配置路径。
...
4. 如果 SSH helper 仍可用，优先执行 `openase machine ssh-bootstrap <machine-id>` 上传当前二进制并刷新与拓扑匹配的服务。
5. 每次迁移后执行 `openase machine test <machine-id>`。
6. 只有在运维仍需 helper 引导或诊断时才保留 SSH 凭证；执行路径本身不会再回退到 SSH。
