...
对于全新的云主机，请把前端编译视为更吃内存的步骤。8 GB RAM 会顺畅得多；如果是 4 GB droplet，建议先加好 swap，再运行 `make build-web`，这样首次构建更稳。
...
在 macOS 本地开发场景下，如果你不想让 Docker 接管数据库生命周期，推荐优先使用 Homebrew 或 Postgres.app 提供的原生 PostgreSQL。只有当你已经依赖 Docker Desktop 或 Colima，且希望 OpenASE 一并管理数据库容器时，再选择 Docker 路径。
...
默认流程在终端内完成；setup 过程本身不会打开浏览器。它将引导你完成：

- 选择数据库来源：
  - 在 Docker 已安装且当前用户可访问 daemon 时，自动启动一个本地 Docker PostgreSQL
  - 手动输入已有 PostgreSQL 的连接字段：`host`、`port`、数据库名、用户名、密码和 `sslmode`
- 验证所选数据库连接
- 检查本地 CLI 可用性和版本探测（`git`、`codex`、`claude` 及其他内置 Provider CLI）
- 选择 OpenASE 设置后的运行方式：
  - 仅配置
  - 在支持的平台上安装/更新当前用户的托管服务：Linux 使用 `systemd --user`，macOS 使用 `launchd`
...
setup 完成后，浏览器登录默认使用本地 bootstrap 链接。如果你之后想启用受管的浏览器登录，请在运行中的实例里开启 OIDC，然后再参考双模式 IAM 契约（[`docs/zh/iam-dual-mode-contract.md`](./iam-dual-mode-contract.md)）和 OIDC 配置指南（[`docs/zh/human-auth-oidc-rbac.md`](./human-auth-oidc-rbac.md)）。运行时适用于 Auth0、Azure Entra ID 等标准 OIDC 提供商。
...
在支持的平台上，这使用仓库的用户服务抽象。服务会读取 `~/.openase/.env`。Linux 下它作为 `systemd --user` 服务运行，`openase logs` 实际读取 `journalctl`；macOS 下它作为 `launchd` LaunchAgent 运行，`openase logs` 会 tail `~/.openase/logs/openase.stdout.log` 和 `~/.openase/logs/openase.stderr.log`。

长期运行时还需要注意：

- 托管服务只负责运行 OpenASE 本身，不负责托管 PostgreSQL。
- 如果你连接的是现成 PostgreSQL，需要你自己保证数据库长期运行。
- 如果 setup 创建了 Docker PostgreSQL 容器，它依然和 OpenASE 服务本身是分离的服务边界。

各平台的服务细节如下：

| 平台 | 管理器 | 定义文件路径 | 查看状态 | 重启 | 停止 | 查看日志 |
|------|--------|--------------|----------|------|------|----------|
...
在 macOS 上，setup 会优先使用 `gui/<uid>/com.openase`，如果当前登录会话挂接在另一个用户域上，则退回到 `user/<uid>/com.openase`。后续执行 `kickstart` 和 `bootout` 时，请使用 `launchctl print` 能成功返回的那个 target。

如果希望 OpenASE 在 Linux 用户退出登录后仍持续运行，通常还需要为该用户启用 linger：
...
# 在 Ubuntu/Debian 的包管理器安装路径下，这一步通常需要 sudo，
# 因为 Node.js 往往装在 root 拥有的前缀目录里。
