...
3. 编排引擎在调度周期中发现可执行的工单，解析其 Workflow 绑定的 Agent 定义，先确保所需 repo mirror 已就绪，再 materialize 本次运行使用的 Workflow / Skills 快照，执行 `on_claim` Hook（派生工单工作副本、密钥解密、依赖准备等），然后创建 AgentRun
...
| 考虑方案 | 优势 | 劣势 | 结论 |
|---------|------|------|------|
| Git 仓库 `.openase/` 作为权威源 | 人类可直接看到文件 | 强依赖 mirror / branch / 工作树状态，控制面资产和代码仓库耦合过深 | 否决 |
| 纯文件系统共享目录 | 简单 | 多机 / 多实例一致性差，恢复与审计弱 | 否决 |
| 数据库存正文 + 版本 / 审计表（采用） | 控制面真相清晰、事务一致、可按版本 materialize 到任意 runtime | 需要自建版本与导出逻辑 | 采用 |
...
| `openase orchestrate` | Go | 工单轮询调度、Agent 进程管理、心跳监控、重试调度、Stall 检测、Workflow/Skill 版本投影 |
...
| Workflow 变更 | serve 写入新的 Workflow / Skill 版本到 DB，orchestrate 读取最新发布版本，不涉及额外进程通信 |
...
`ProjectRepoMirror` 表示某个 `ProjectRepo` 在某台执行机器上的项目级本地镜像。它是代码 checkout、Ticket 工作区派生、代码级 Hook 执行的本地真相源，但**不再**是 Workflow / Skill 内容的权威源。
...
- `ProjectRepoMirror` 是代码仓库本地镜像，而不是 Workflow / Skill 的内容真相源。
- 任何需要“项目代码仓库根目录”的操作，都必须基于某个 `ready` 的 mirror，而不是直接把 `repository_url` 当本地路径解析。
- 平台必须显式区分“repo 已绑定”与“某个 repo mirror 已就绪”；前者不代表后者。
...
- Workflow 编辑、发布、技能绑定变更一律通过 Platform API 完成；Agent 不允许直接修改 repo mirror 中的 `.openase/` 文件来改变平台行为。
- 任何读取 / 编辑 Workflow 与 Skill 的控制面操作，不再依赖任何特定 repo mirror 是否 ready。mirror 只影响代码仓库相关操作，不影响 Workflow / Skill 的查看、编辑、版本化与绑定。
...
**Workflow Hook 的执行上下文不是工单工作区**——它在服务控制的 project-level runtime context 中执行。若 Hook 需要访问代码仓库，平台可显式挂载 Hook 声明所需的 repo mirror 或轻量 project workspace；但 Workflow/Skill 的控制面编辑本身不依赖 mirror readiness。

**前置条件：**

- Workflow Hook 的触发来自控制面中的 Workflow 生命周期事件，而不是仓库 `.openase/harnesses` 文件监听
- 打开、编辑、发布 Workflow 与 Skills 不要求任何 repo mirror 处于 ready
...
- `ticket.on_claim` 不负责创造“所需 repo mirror 是否存在”这一前提；若 mirror 缺失，调度器必须先走 `prepare_mirror`
...
| 面板 | 说明 |
|------|------|
| 项目状态摘要 | 项目健康状态、默认 Workflow、已绑定 Repo 数、活跃状态列数 |
...
如果部分步骤失败但不阻塞进入系统，例如默认 Workflow 模板初始化失败：

- 成功页仍可进入系统
- 但展示黄色提示条：`平台默认 Workflow / Skills 尚未初始化完成，请按提示重试初始化`
- 提供一键重试或展开查看补救步骤
...
3. 初始化平台默认 Workflow / Skills 模板与内置 Skill 元数据
...
| 区域 | 内容 |
|------|------|
| 顶部欢迎条 | 当前项目名、已启用 Provider、Repo 连接状态 |
...
- 单 Repo 项目自动带出唯一仓库；多 Repo 项目要求显式选择 repo scope
- 表单底部展示“创建后会发生什么”：
  - 工单进入默认 pickup 列
  - 编排引擎在下一个 Tick 检测到
...
| **Harness 内容** | OpenASE DB | PostgreSQL `workflow_versions.*`（或等价版本表） | serve (API) / refine-harness Agent | orchestrate（runtime materializer / scheduler） |
...
| Harness 内容 | 新版本发布到调度器读取之间（通常 < 5s） | orchestrate 定期读取最新发布版本 | 单次读取失败：下个 Tick 重试 |
...
```
                 信源：OpenASE DB + Git 仓库
                   ┌─────────────┐
                   │ Harness 版本 │──publish───→ orchestrate/runtime materializer
                   │ Skill 版本   │
                   └─────────────┘
                           │
                           │
...
hooks:
  on_claim:
    - cmd: "git fetch origin && git checkout -b {{ git.branch_pattern }} origin/{{ repos[0].default_branch }}"
      timeout: 60
    - cmd: "pnpm install --frozen-lockfile"
      workdir: "frontend"
      timeout: 300
...
Skill 是 Agent CLI（Claude Code、Codex 等）原生支持的能力扩展单元。每个 Skill 仍然以 `SKILL.md` 语义存在，但在 OpenASE 中它首先是一个**平台控制面资产**，而不是项目仓库中的目录。
...
Skill 对 Agent 来说是“你会什么”；Harness 是“你的角色是什么”。**Workflow 绑定 Skills = 这个角色在本次运行中拥有哪些能力。**

### 34.2 最新设计与废弃说明

OpenASE 现采用 **DB 权威源 + runtime materialize** 模型：

- Skill 正文、元数据、启用状态、绑定关系都存储在平台控制面
- Agent / 用户若要修改 Skill，必须调用 Platform API
- 新 runtime 创建时，平台把当前版本的 Skills materialize 到工作区中的 Agent CLI skills 目录
- 已运行中的 runtime 不会因为后台 Skill 更新而自动漂移；只有新 runtime 默认拿最新版本，或显式 refresh / restart

> **废弃设计**：历史设计中，Skills 存储在项目仓库 `.openase/skills/`，并通过 `skills refresh` / `skills harvest` 在 repo 与工作区之间双向同步。该设计现已废弃。repo mirror、branch、工作树状态不再决定 Skill 的权威内容。

### 34.3 存储模型

Skill 由平台控制面持久化，推荐最小模型如下：
...
| content_markdown | Text | `SKILL.md` 正文快照 |
...
3. 取出这些 Skill 对应的正文版本
4. 写入本次 workspace 的 Agent CLI skills 目录：
   - Claude: `.claude/skills/<skill>/SKILL.md`
   - Codex: `.codex/skills/<skill>/SKILL.md`
   - 其他: `.agent/skills/<skill>/SKILL.md`
...
- mirror readiness 不影响 Skill 的查看、编辑、启用、绑定；只影响代码仓库相关执行
...
- `refresh current runtime skills`
  - 从平台控制面重新读取当前绑定 Skill 的**最新已发布版本**
...
| 操作 | 说明 |
|------|------|
| 创建 | 通过 Web UI / CLI / Agent Platform API 创建 Skill |
| 更新 | 提交新的 Skill 正文，生成新版本 |
...
| GET | `/api/v1/skills/:skillId` | 获取 Skill 详情（含当前版本内容） |
| PUT | `/api/v1/skills/:skillId` | 更新 Skill 内容，生成新版本 |
...
写入 Skill 时严格校验，格式错误的请求直接拒绝，不影响其他 Skill 和 Agent 正常运行：

| 检查项 | 失败行为 |
|--------|---------|
| Skill 名称为空或不合法 | `400 Bad Request` |
| 正文为空 | `400 Bad Request` |
| 文件大小 > 100KB | `400 Bad Request` |
| YAML Frontmatter 语法错误（如果有） | `400 Bad Request` |
...
| Orchestrator Worker | 启动 Agent 前执行 Skill materialize；不再做 repo harvest |
| Infrastructure | 新增 `infra/skill/`（版本存储、runtime materialize、格式校验） |
...
| 数据库 | 新增 `skills`、`skill_versions`、`workflow_skill_bindings` |
...
| ID | 任务 | 工作量 | 依赖 | PRD 章节 |
|----|------|--------|------|---------|
| F28 | **Setup Wizard**（4 步引导 + 模式选择 + DB 测试连接 + 项目与默认 Workflow 初始化） | 5d | F04, F07, F10, F24 | 14 |
