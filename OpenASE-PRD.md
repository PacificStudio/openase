...
前端 UI 参考 Linear 的简洁风格，核心页面：

| 页面 | 功能描述 |
|------|---------|
| 项目列表 | 所有项目概览，状态、工单统计 |
| 工单看板 | Linear 风格的 Kanban 视图 + 列表视图 |
| 工单详情 | 状态、描述、Workflow、Agent、多 Repo PR 状态列表、活动日志、依赖关系、Hook 执行历史 |
| Workflow 管理 | Workflow 列表、Harness Git-aware 编辑器、版本历史、Diff 视图 |
| Agent 控制台 | Agent 状态总览、实时输出流、心跳监控、成本统计 |
| 审批中心 | 待审批工单列表、Hook 执行结果、多 Repo PR 状态、Reviewer 操作界面 |
| 定时任务 | 定时任务配置和执行历史 |
| 活动时间线 | 所有操作的审计日志 |
| 成本仪表盘 | 按项目 / Workflow / Agent 维度展示 Token 消耗和成本趋势 |
| 编排引擎状态 | Orchestrator 运行状态、Worker 池、调度队列、重试队列 |
| 设置 | AgentProvider 管理、Git 集成配置、项目配置 |
