...
ticket_hooks:
  on_claim:
{% for repo in repos %}
    - cmd: "git fetch origin && git checkout -B agent/{{ agent.name }}/{{ ticket.identifier }} origin/{{ repo.default_branch }}"
      workdir: "{{ repo.name }}"
      timeout: 60
{% endfor %}
...
| **Skill 内容** | OpenASE DB | PostgreSQL `skill_versions.*`（或等价版本表） | serve (API) / Agent Platform API | orchestrate（runtime materializer；在 runtime 创建时直接读取已发布版本） |
