...
- `pickup_status_ids` 只能引用 `stage ∈ {backlog, unstarted, started}` 的 TicketStatus；`finish_status_ids` 只能引用 `stage ∈ {completed, canceled}` 的 TicketStatus。
...
中间工单在什么状态、用户怎么拖拽、看板有几列——编排引擎不关心具体名字，但会读取结构化 `stage` 来判断哪些状态是入口态、哪些状态是终态，以及依赖是否已经解除。
...
- 标题输入框自动聚焦
- 自动推荐一个初始 Workflow（通常是 `coding`），并明确展示它的 pickup 状态
- 单 Repo 项目自动带出唯一仓库；多 Repo 项目要求显式选择 repo scope
- 表单底部展示“创建后会发生什么”：
  - 工单进入所选 Workflow 的 pickup 列
  - 编排引擎在下一个 Tick 检测到
...
connectors:
  - type: github
    ...
    config:
      ...
      auto_workflow: "coding"                      # 同步进来的 Issue 初始写入 ticket.workflow_id = coding；实际领取仍由 pickup 状态决定
```
...
Stage 的设计目标不是限制用户拖拽，而是消除后端对 `"Done"` 这类状态名的魔法依赖。依赖解除、终态判断、Dashboard 统计、Workflow 合法性校验都必须基于 `stage`，而不能基于状态名称猜测语义。
...
- `pickup_status_ids` 只能绑定 `stage ∈ {backlog, unstarted, started}` 的状态
- `finish_status_ids` 只能绑定 `stage ∈ {completed, canceled}` 的状态
...
Agent 在 Prompt 中看到有语义的 Custom Status 名称（"文献调研"比"in_progress"有用得多），编排引擎通过 `pickup_status_ids` 和 status 对应的 `stage` 做结构化判定，而不是通过状态名猜语义。
...
| 第七章 已重写 | 不再有硬编码状态机；编排引擎看 Workflow 的 pickup_status_ids，并通过 stage 判断入口态 / 终态 |
...
核心简化：**没有硬编码状态迁移图，但有结构化 Stage 语义。** 编排引擎只匹配 `status_id ∈ pickup_status_ids`，状态流转仍由用户（看板拖拽）和 Agent（完成时移到允许的 finish 状态之一）驱动；列顺序由 `TicketStatus.position` 决定，并发上限直接由 `TicketStatus.max_active_runs` 表达。Stage 只负责统一回答“这个状态是否属于入口态 / 终态”，以及依赖和统计该如何解释它。
...
3. **约束与控制面**
   ...
   - Workflow create/update 校验：
     - `pickup_status_ids` 只能选 `backlog/unstarted/started`
     - `finish_status_ids` 只能选 `completed/canceled`
