...
- **Webhook 监听**：为 Project 下所有 ProjectRepo 注册 Webhook，监听 PR 事件（opened、review_requested、merged、closed），通过分支名中的工单标识（如 `agent/ASE-42`）自动匹配到对应的 TicketRepoScope，更新其 pr_status
...
func (s *Service) SyncPullRequestStatus(ctx context.Context, input PullRequestStatusInput) error {
    // 1. 从 branch name 解析工单标识：agent/ASE-42 → ASE-42
