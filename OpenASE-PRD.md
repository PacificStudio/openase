...
**ActivityEvent 必须保持 coarse-grained。允许进入这一层的典型事件：**

- `ticket.created` / `ticket.updated` / `ticket.status_changed`
- `agent.claimed` / `agent.completed` / `agent.failed` / `agent.terminated`
- `hook.started` / `hook.passed` / `hook.failed`
- `pr.opened` / `pr.merged` / `pr.closed`
- `ticket.retry_scheduled` / `ticket.budget_exhausted`
...
- Ticket 详情页的 System Activity：只读取按 `ticket_id` 过滤后的 `ActivityEvent`
- Ticket 详情页的 Comments：只读取 `TicketComment`
...
5. Lifecycle / Business Projector 只在发生 coarse-grained 业务事件时写入 `ActivityEvent`，例如 `agent.claimed`、`agent.completed`、`hook.failed`、`pr.opened`。
...
func (s *Scheduler) runTick(ctx context.Context) {
    ...
    for _, wf := range workflows {
        ...
        for _, t := range candidates {
            ...
            // 以及“当前命中的 pickup status 所属 Stage”是否还有容量
...
func (s *Scheduler) canDispatch(ctx context.Context, t *ticket.Ticket, wf *workflow.Workflow) bool {
    ...
    // 当前命中的 pickup status 所属 Stage 并发已满 → 跳过
    if matchedPickupStage.MaxActiveRuns > 0 &&
       s.pool.ActiveCountForStage(matchedPickupStage.ID) >= matchedPickupStage.MaxActiveRuns {
        return false
    }
    // 并发数已满 → 跳过
...
4. 检查分发门槛：
   ...
   - 当前命中的 pickup status 所属 Stage 若配置了 `max_active_runs`，则该 Stage semaphore 未满
   - per-workflow 并发未满
...
**明确要求：系统必须支持并行。** 这里的并行既包括“多个不同 Agent 定义并行处理多个 Ticket”，也包括“同一个 Agent 定义在 Provider / Workflow / Stage / Global 并发限制允许时，同时产生多个 `AgentRun` 来处理多个 Ticket”。禁止把 Agent 误实现成“同一时刻只能占用一个 Ticket”的单 worker 槽位。
...
| **项目/工单业务活动流** | OpenASE DB | PostgreSQL `activity_events.*` | serve / orchestrate / webhook / hook executor | serve (dashboard / ticket activity / project activity) |
...
test-backend-coverage: ## 运行后端覆盖率门禁（overall 85%+ / domain+types 100%）
...
编排引擎查询语义变为：`WHERE status_id IN workflow.pickup_status_ids AND current_run_id IS NULL AND retry_paused = false AND (next_retry_at IS NULL OR next_retry_at <= now())`。命中候选工单后，还需检查其当前命中的 pickup status 所属 `TicketStage.max_active_runs` 是否已满。没有 fail 状态，没有硬编码状态机，Agent 出错就退避重试。
...
默认的 coding Workflow 自动配置 `pickup_status_ids = ["Todo"]`、`finish_status_ids = ["Done"]`。用户修改看板列后只需要对应修改 Workflow 的状态集合配置即可。
...
Agent 在 Prompt 中看到有语义的 Custom Status 名称（"文献调研"比"in_progress"有用得多），编排引擎通过 `pickup_status_ids` 匹配而非状态名。
...
| 章节 | 影响 |
|------|------|
| 第六章 领域模型 | Ticket.status 改为 status_id FK；新增 TicketStage / TicketStatus；Workflow 新增 pickup_status_ids / finish_status_ids |
| 第七章 已重写 | 不再有硬编码状态机；编排引擎看 Workflow 的 pickup_status_ids，并可受 Stage 并发限制约束 |
| 第八章 Hook | 触发点不再引用具体状态名，只关心编排生命周期事件 |
| 第十章 编排引擎 | 调度查询改为 `WHERE status_id IN workflow.pickup_status_ids AND current_run_id IS NULL`，并额外检查匹配状态所属的 Stage semaphore |
...
核心简化：**没有硬编码状态机。** 编排引擎只匹配 `status_id ∈ pickup_status_ids`，不做任何状态转换合法性校验；Stage 只负责 UI 分组和共享并发上限，不参与跳转合法性判断。状态流转完全由用户（看板拖拽）和 Agent（完成时移到允许的 finish 状态之一）驱动。出错不改状态，只是退避重试。
...
| 持久化 | `AgentTraceEvent + AgentStepEvent + ActivityEvent` 分层持久化 | 不持久化（或仅记入项目级日志） |
| 并发 | 支持多个 Agent 定义并行执行多工单；同一个 Agent 定义在并发限制允许时也可同时驱动多个 AgentRun | 每用户最多 1 个会话 |
