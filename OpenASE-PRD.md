...
- 将 Agent 的行为定义（Harness）纳入版本控制，使其可审计、可回滚
...
**原则 3：行为即代码** — Agent 的行为定义（Harness）存储在 Git 仓库中，而非数据库。修改 Harness 就是提交 Git Commit，可以被 Diff、Review 和 Rollback。这让 Agent 的行为与人类代码一样可审计。
...
每个 Workflow 的核心是一个 Harness 文件，存储在项目仓库的 `.openase/harnesses/` 目录中。Harness 用 YAML Frontmatter + Markdown 格式定义 Agent 的工作规范。

**Harness 的核心特性：**

- **热重载**：Harness 文件修改后即时生效，新工单自动使用最新版本
- **Git 原生**：Harness 变更就是 Git Commit，可 Diff、Review、Rollback
...
**决策 3：Harness 存储在 Git 仓库**

| 考虑方案 | 优势 | 劣势 | 结论 |
|---------|------|------|------|
| 数据库 Text 字段 | 简单统一 | 无法 Diff/Review、版本管理需自建 | 否决 |
| 文件系统 (Symphony 方案) | 简单 | 多实例部署需共享存储 | 部分采纳 |
| Git 仓库 + DB 索引 (采用) | 原生 Diff/Review/Rollback | 需要 Git 操作层 | 采用 |
...
| 服务 | 语言 | 职责 |
|------|------|------|
| `openase serve` | Go | 工单 CRUD、状态机、Workflow 管理、SSE 推送、Git Webhook 接收、认证 |
| `openase orchestrate` | Go | 工单轮询调度、Agent 进程管理、心跳监控、重试调度、Stall 检测、Harness 热重载 |
...
| Workflow 变更 | HarnessWatcher 通过 fsnotify 监控文件系统，不涉及进程通信 |
| GitHub PR 事件 | serve 自己接收 Webhook、更新 TicketRepoScope、推进状态，不需要通知编排引擎 |
| Agent 注册/配置 | serve 写入数据库，编排引擎下次 SelectAgent 时自然读到 |
...
项目 Git 仓库中的配置目录（版本控制，不含敏感信息）：

```
your-project/
├── .openase/
│   ├── harnesses/             # Workflow Harness 文件（含 Hook 定义）
│   │   ├── coding.md
│   │   ├── test.md
│   │   └── security.md
│   └── skills/                # Skill 文件
...
| default_workflow_id | FK | 默认 Workflow 模板 |
...
| 关系类型 | 说明 | 行为 |
|---------|------|------|
| blocks | A blocks B：A 完成前 B 不能开始 | B 状态锁定在 backlog，直到 A 达到 done |
...
| title | String | 外部 Issue/PR 的标题（缓存，Webhook 更新时刷新） |
| status | String | 外部状态（open / closed / merged，Webhook 更新时刷新） |
...
- Webhook 回调时通过 `external_id` 匹配更新 `status`，工单详情页实时展示所有关联 Issue/PR 的最新状态

**Harness 模板变量**：

```
{{ range .ExternalLinks }}
- [{{ link.type }}] {{ link.title }} ({{ link.status }}): {{ link.url }}
{{ end }}
```

**工单仓库作用域（TicketRepoScope）：**

一个工单可以涉及 Project 下的一个或多个 Repo。TicketRepoScope 记录了每个工单在每个相关 Repo 中的具体 Git 状态。
...
| pr_status | Enum | none / open / changes_requested / approved / merged / closed |
| ci_status | Enum | pending / passing / failing |
| is_primary_scope | Boolean | 是否为该工单的主 Repo（工单状态主要跟踪此 Repo 的 PR） |

**工单状态与多 Repo PR 的关系：**

- 工单只有当**所有** TicketRepoScope 的 PR 都 merged 后才能转为 done
- 任何一个 Repo 的 PR 被 close without merge → 触发 on_error Hook，清空 assigned_agent_id，指数退避后重试（Agent 可重新创建 PR）
- 任何一个 Repo 的 PR 收到 changes_requested，工单保持 in_review，Agent 继续处理
- `on_complete` Hook 聚合所有 Repo 的 CI 状态——只有全部 passing 才允许推进到 in_review
...
- backend (Go API): /workspaces/ASE-42/backend/
- frontend (SvelteKit): /workspaces/ASE-42/frontend/
- sdk (TypeScript): /workspaces/ASE-42/sdk/

请在所有相关仓库中完成必要的修改，并为每个仓库创建独立的 PR。
```

**分支命名规范**：每个 Repo 中的分支统一命名为 `agent/{agent-name}/{ticket-identifier}`，确保跨 Repo 的分支可追溯到同一个工单。
...
| harness_path | String | Harness 文件在 Git 仓库中的路径 |
...
### 7.2 Workflow 定义 Pickup 规则

每个 Workflow 在 Harness 中声明两个关键状态：

```yaml
---
# 在 Harness YAML Frontmatter 中
status:
  pickup: "Todo"           # 工单处于此状态时，编排引擎接手执行
  finish: "Done"           # Agent 完成后，工单移到此状态
  # 无 fail 状态——失败自动重试，人类 cancel 才终止           # Agent 失败后，工单移到此状态（可选，默认不改状态）
---
```
...
func (s *Scheduler) runTick(ctx context.Context) {
    ...
    for _, wf := range workflows {
        // 每个 Workflow 有自己的 pickup status
        pickupStatusID := wf.PickupStatusID

        // 扫描处于 pickup status 的工单
        candidates, _ := s.ticketRepo.ListByStatusID(ctx, wf.ProjectID, pickupStatusID)
...
**就这么简单。** 没有硬编码状态机。编排引擎只做三件事：

1. **扫描**：找到 `status == workflow.pickup` 的工单
2. **执行**：启动 Agent，跑 Hook
3. **结束**：Agent 完成 → 把工单移到 `workflow.finish`；Agent 出错 → 指数退避后自动重试（不存在 "failed" 终态）
...
中间工单在什么状态、用户怎么拖拽、看板有几列——编排引擎完全不管。
...
```yaml
status:
  pickup: "Todo"           # 用户把工单拖到 Todo 列 → 编排引擎接手
  finish: "Done"           # Agent 完成 → 工单跳到 Done 列
...
编排引擎在执行过程中需要知道工单是否正在被处理（避免重复分发）。这不通过 Custom Status 实现，而是通过 Ticket 的 `assigned_agent_id` 字段：

- `assigned_agent_id == null`：未被领取，可以分发
- `assigned_agent_id != null`：已被领取，跳过
...
| Hook | 触发时机 | 典型用途 |
|------|---------|---------|
| `workflow.on_activate` | Harness 首次加载或热重载成功时 | 验证依赖环境（Agent CLI 是否可用、Git 仓库可达性）；预热缓存 |
| `workflow.on_deactivate` | Workflow 被禁用或 Harness 删除时 | 清理全局资源；通知团队该角色已下线 |
| `workflow.on_reload` | Harness 文件变更触发热重载时 | 验证新配置合法性；通知团队 Harness 已更新 |
...
**Ticket Hook 在工单工作区目录下执行**（即 `on_claim` 创建的 `/workspaces/ASE-42/` 目录）。
...
| `OPENASE_WORKSPACE` | 工单工作区根目录 | `/workspaces/ASE-42` |
| `OPENASE_REPOS` | 仓库列表 JSON | `[{"name":"backend","path":"/workspaces/ASE-42/backend"}]` |
...
工单 ASE-42 被领取
  ...
  │
  └── ticket.on_done Hook（清理、通知、Skill 收割）
```
...
| 指标名 | 类型 | 标签 | 说明 |
|--------|------|------|------|
| `openase.agent.active` | Gauge | project, provider, adapter_type | 当前活跃 Agent 数 |
| `openase.agent.utilization_ratio` | Gauge | project, provider | Agent 利用率（running / total） |
...
| `openase.orchestrator.harness_reload_total` | Counter | — | Harness 热重载次数 |
...
| Agent 利用率 | 各 Agent 的忙碌/空闲比例、当前任务 |
...
1. **对账**：检查所有 running 状态的工单——Agent 是否还在运行？是否 stall？工单是否被取消？
2. **验证 Harness**：检查 Harness 文件是否有变更（mtime + hash），变更则热重载
...
| 场景 | 退避策略 | 说明 |
|------|---------|------|
| Agent 正常退出 | 1 秒快速重试 | 可能是 Turn 完成但工单未结束 |
| Agent 异常退出 | 10s × 2^(attempt-1)，上限 5 分钟 | 指数退避 |
...
- **Webhook 监听**：为 Project 下所有 ProjectRepo 注册 Webhook，监听 PR 事件（opened、review_requested、merged、closed），通过分支名中的工单标识（`agent/*/ASE-42`）自动匹配到对应的 TicketRepoScope，更新其 pr_status
- **PR 绑定工单**：PR 描述中包含工单标识（如 `Fixes ASE-42`），自动建立 TicketRepoScope 关联
- **多 Repo PR 聚合**：当一个工单涉及多个 Repo 时，前端工单详情页展示所有 Repo 的 PR 状态列表，清晰标识哪些已合并、哪些待 Review
- **GitHub Issue 同步**：从 GitHub Issue 自动创建工单，根据 Issue 所在的 Repo 自动设置 TicketRepoScope
- **CI 状态回传**：每个 Repo 的 CI 结果独立回传到对应的 TicketRepoScope 的 `ci_status` 字段，`on_complete` Hook 可读取聚合状态做判断
...
| in_review | 所有涉及 Repo 的 PR 已开启 | Agent 为每个 Repo 提交 PR |
| in_review | 任一 PR changes_requested | Reviewer 要求修改，Agent 继续处理 |
| done | **所有** PR merged | 全部 Repo 的 PR 合并成功 |
| （重试/暂停） | 任一 PR closed without merge | 记录错误，清空 assigned_agent_id，指数退避后重试（Agent 可以重新创建 PR） |
...
| 实体层 | Ticket、Harness、Approval、Connector、Channel | Ticket 详情、Harness 编辑器、审批详情 |
...
| 新建按钮 | 支持下拉：新建工单 / 新建项目 / 新建 Workflow / 新建 Connector |
...
| Running Now | 当前运行中的 Agent 数、活跃工单数、待审批数 |
...
| 面板 | 说明 |
|------|------|
| 项目状态摘要 | 项目健康状态、默认 Workflow、主仓库、活跃状态列数 |
...
| 当前 Agent | 有则显示 | `claude-01` / `codex-02` |
| Repo/PR 状态 | 有则显示 | `2 PRs`, `1 merged`, `changes requested` |
...
| Summary | 描述、Workflow、当前 Agent、涉及 Repo |
...
Ticket Detail 必须支持类似 GitHub Issue 的评论线程，作为用户和 Agent 围绕单个工单协作的主讨论面。

**当前阶段范围：**

- 在 Ticket 详情抽屉底部提供评论输入区
- 已发布评论支持 Markdown 渲染
- 用户可对已有评论执行编辑、删除
- 评论按时间正序展示，新的评论追加在底部
- 系统事件时间线与用户评论分开展示，避免混在一起影响可读性

**评论卡片字段：**

| 字段 | 说明 |
|------|------|
| 作者 | 当前阶段先显示 `created_by` 文本标识 |
| 发布时间 | 相对时间 + 悬浮可显示绝对时间 |
| 评论正文 | Markdown 渲染后的正文 |
| 编辑状态 | 若评论被修改，显示 `edited` 标记 |
| 操作 | `Edit` / `Delete` |

**编辑交互：**

- 点击 `Edit` 后，评论正文切换成 textarea
- 编辑态必须有 `Save` 和 `Cancel`
- `Save` 成功后原地刷新为渲染结果
- `Delete` 需要二次确认或明确 destructive affordance
...
- 评论线程是主讨论面，默认展示用户可读正文
- Activity 继续承载状态变化、Agent 生命周期、Hook 结果等系统事件
- 评论新增/编辑/删除后，详情抽屉应实时刷新，避免用户手动重开
...
Agent 页面分成“实例视角”和“配置视角”两层，避免把运行状态和 Provider 配置混在一起。
...
| 字段 | 说明 |
|------|------|
| Agent 名称 | 如 `claude-01` |
| Provider / Model | Claude Code / Codex / Gemini |
| 当前状态 | idle / running / offline / stalled |
...
| Capabilities | 能力标签、适用 Workflow、机器绑定 |
...
| Agents | 运行控制台 | Agent 实例、心跳、输出、Provider 配置 |
| Approvals | 风险治理 | 待审批列表、上下文、通过/拒绝 |
| Activity | 审计与回放 | 跨实体事件时间线 |
| Insights | 运营分析 | 成本、吞吐、完成率、重试、SLA |
| Settings | 配置中心 | 项目、Repo、状态列、Connector、通知、机器、安全 |
...
📋 创建你的第一个项目
  ...
  Git 仓库:
  ┌──────────────────────────────────────────────────────────────┐
  │  ⭐ backend   https://github.com/you/backend.git   [主仓库]  │
  │  ✚ frontend  https://github.com/you/frontend.git            │
  │                                                    [添加仓库]│
  └──────────────────────────────────────────────────────────────┘

  第一个添加的仓库自动标记为主仓库（⭐），可点击切换。
  主仓库用于存放 .openase/ 配置目录（Harness + Skills）。
...
**"主仓库"的含义：** `.openase/harnesses/` 和 `.openase/skills/` 目录存放在主仓库的根目录中。其他仓库不存放 OpenASE 配置。多 Repo 工单执行时，编排引擎从主仓库的 `.openase/` 中读取 Harness 和 Skills。
...
- 项目名称输入框置顶
- 仓库列表作为可编辑表格或卡片列表展示
- 识别主仓库后，在右侧显示“将创建 .openase/ 目录”的说明
- 检测到仓库语言后，立即在底部显示“推荐 Harness 模板”
...
- 检测到 Go 主仓库 → 推荐 `coding + testing`
- 检测到前后端混合仓库 → 推荐 `coding + testing + docs`
...
Setup Wizard 中存在多处异步动作：测试数据库、检测 Agent、识别仓库语言、初始化 `.openase/`、安装服务、启动系统。
...
- 用户不需要重复填写已成功的步骤
- 部分成功要明确展示，例如“配置文件已写入，但 push `.openase/` 失败”
...
如果部分步骤失败但不阻塞进入系统，例如主仓库 push 失败：

- 成功页仍可进入系统
- 但展示黄色提示条：`主仓库中的 .openase/ 文件尚未推送，请按提示手动提交`
- 提供一键复制命令或展开查看补救步骤
...
3. **Clone 主仓库到临时目录 → 生成 `.openase/` 目录结构（harnesses + skills）→ Git commit + push**
   - 如果主仓库已有 `.openase/` 目录 → 跳过生成，使用已有配置
   - 如果推送失败（无权限等）→ 提示用户手动 commit 生成的文件，不阻塞后续步骤
...
| 项目配置 | 单 Repo 简化流程 | 多 Repo 向导（含主仓库选择） | 多 Repo + 治理策略 |
...
Setup Wizard 完成后，主界面会显示一个引导面板，鼓励用户创建第一个工单。支持两种方式：

**首次进入主应用的页面过渡：**

- Wizard 成功页点击“进入 OpenASE”后，先进入项目级 Dashboard
- Dashboard 顶部显示欢迎条：`你的工作区已经准备好了，接下来创建第一个工单`
- 左侧边栏已自动聚焦刚创建的项目
- 主区域中的空状态卡片直接说明“推荐先做什么”

**首次进入 Dashboard 的推荐布局：**

| 区域 | 内容 |
|------|------|
| 顶部欢迎条 | 当前项目名、已启用 Provider、主仓库状态 |
| First Ticket 面板 | 主 CTA：创建第一个工单 |
| Board Snapshot | 初始列为空，但显示每一列的用途说明 |
| Getting Started Checklist | 已连接数据库、已检测 Agent、已创建项目、待创建首个工单 |
| 帮助入口 | 查看示例 Harness、打开文档、复制 CLI 示例 |

**方式一：Web UI 创建**

主界面顶部显示引导横幅："创建你的第一个工单，让 Agent 开始工作！" 点击后进入工单创建表单，自动推荐 Workflow 和涉及 Repo。

**方式二：CLI 创建**

```bash
openase ticket create --title "Add input validation to login form" --workflow coding
openase watch ASE-1  # 终端实时观察 Agent 工作
```

无论哪种方式，创建后用户可以在 Web UI 或终端实时看到 Agent 的工作进度。

**首个工单创建表单的 UX 要求：**

- 标题输入框自动聚焦
- 自动推荐默认 Workflow（通常是 `coding`）
- 自动带出主仓库
- 表单底部展示“创建后会发生什么”：
  - 工单进入默认 pickup 列
  - 编排引擎在下一个 Tick 检测到
  - Agent 开始执行并通过 SSE 回传进度

**首个工单创建后的引导反馈：**

- 看板自动切换到包含该工单的列
- 该卡片短暂高亮，帮助用户定位
- 右下角出现轻提示：`Agent 开始工作后，你会在这里看到实时进度`
...
func (s *Scheduler) runTick(ctx context.Context) {
    ...
    // ── Step 2: 检查 Harness 热重载 ──
    if s.harnessWatcher.HasChanges() {
        reloaded := s.harnessWatcher.Reload()
        s.metrics.Counter("openase.orchestrator.harness_reload_total").Add(len(reloaded))
    }
...
### 16.4 路径四：on_complete Hook 执行 + 状态推进
...
GitHub  ──POST /api/v1/webhooks/github──→  httpapi webhook handler  ──→  ticket/workflow service
                                           ...
                                           ▼
                                     匹配 branch name
                                     agent/claude-01/ASE-42
                                           │
                                           ▼
                                   更新 TicketRepoScope.pr_status
                                           │
                                           ▼
                                   检查是否所有 PR 都 merged
                                           │
...
func (s *Service) SyncPullRequestStatus(ctx context.Context, input PullRequestStatusInput) error {
    ...
    for _, s := range allScopes {
        if s.PRStatus != "merged" { allMerged = false }
        if s.PRStatus == "closed" { anyClosed = true }
    }
...
**通知方式：**
- GitHub → Webhook HTTP POST → 同步处理
- PR merged → `EventProvider.Publish("ticket.events", DoneEvent)` → SSE → 前端更新
- changes_requested → `EventProvider.Publish(ChangesRequestedEvent)` → 编排引擎 Worker 收到 → 给 Agent 发续接 Prompt
...
| **PR 状态缓存** | OpenASE DB | PostgreSQL `ticket_repo_scopes.pr_status` | serve (Webhook handler) | serve (API/SSE)、orchestrate |
| **CI 状态** | Git 平台 CI | GitHub Actions / GitLab CI | CI 系统 (Webhook) | serve (Webhook handler → DB) |
| **CI 状态缓存** | OpenASE DB | PostgreSQL `ticket_repo_scopes.ci_status` | serve (Webhook handler) | on_complete Hook (读 DB) |
...
|------|-----------|------------|---------|
| PR 状态缓存 | Webhook 到达前（通常 < 5s） | GitHub Webhook 推送 → 更新 DB | Webhook 丢失：定时轮询 Git API 补偿（Phase 2 实现 reconciler） |
| CI 状态缓存 | CI 完成到 Webhook 到达之间 | 同上 | 同上 |
| Harness 内容 | git push 到 fsnotify 检测之间（< 1s） | HarnessWatcher 持续监控 + Tick 时校验 hash | 文件系统事件丢失：每次 Tick 强制 hash 对比 |
...
**风险 3：Webhook 丢失导致 PR 状态过时**

- 场景：GitHub Webhook 发送失败（网络问题、OpenASE 宕机期间）
- 信源冲突：GitHub 上 PR 已 merged，但 DB 中 pr_status 还是 open
- 防御：Phase 2 实现 PR Reconciler——定时（每 10 分钟）轮询所有 in_review 工单关联的 PR 最新状态，更新 DB
- 补偿：`openase reconcile --prs` 手动触发全量 PR 状态同步
...
openase reconcile --prs        # 只对账 PR 状态
...
```
                    信源：Git 仓库
                   ┌─────────────┐
                   │ Harness 文件  │──fsnotify──→ HarnessWatcher (缓存)
                   │ Hook 脚本    │
...
1. **信源唯一**：每条信息只有一个写入者。工单状态只有状态机写，PR 状态只有 Webhook handler 写，Harness 只有 Git 仓库存储
2. **缓存可重建**：OpenASE DB 中所有从外部系统缓存的数据（PR 状态、CI 状态、用户信息）都可以从信源重新拉取重建
3. **乐观锁防双写**：两个进程可能同时写同一条工单时，用 `version` 字段做乐观并发控制
4. **Reconciler 兜底**：即使即时通知（Webhook、NOTIFY）丢失，后台 Reconciler 定时对账保证最终一致
...
| GET | `/api/v1/tickets/:ticketId` | 工单详情（含 RepoScopes、依赖、活动日志） | |
...
| GET | `/api/v1/workflows/:workflowId/harness` | 获取 Harness 原始内容（Markdown） |
| PUT | `/api/v1/workflows/:workflowId/harness` | 更新 Harness（触发 Git commit） |
| GET | `/api/v1/workflows/:workflowId/harness/history` | Harness 版本历史（Git log） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/webhooks/github` | GitHub Webhook 接收（PR 事件、CI 状态） |
| POST | `/api/v1/webhooks/gitlab` | GitLab Webhook 接收（Phase 4） |
...
| 磁盘空间不足 | 编排引擎 Tick 时检查 `~/.openase/workspaces/` 所在分区剩余空间；低于阈值（默认 1GB）时暂停分发新工单 |
...
工单在优雅关机后保持原状态（assigned_agent_id 被清空）。下次启动时，编排引擎通过崩溃恢复流程扫描到这些工单并重新分发。
...
-- Agent 能力标签查询（TEXT[] 原生数组，GIN 索引支持 @> 包含查询）
...
以下是一个生产级别的 Coding Harness 完整示例，存储在 `.openase/harnesses/coding.md`：
...
# ═══ 分支规范 ═══
git:
  branch_pattern: "agent/{{ agent.name }}/{{ ticket.identifier }}"
...
orchestrator:
  ...
  workspace_root: "~/.openase/workspaces"  # 工作区根目录
...
→ Agent 完成 → on_complete Hook → 状态推进到 in_review
```
...
测试环境：真实 PostgreSQL + mock GitHub Webhook（HTTP 调用）
...
**整体覆盖率目标：85%+，其中 domain 层 100%。**

不追求整体 100%——那会导致为了覆盖率写无意义的测试（比如测试 getter 方法）。用 `go test -coverprofile` 在 CI 中追踪，设 85% 为门槛，domain 层 100% 为硬性要求。
...
| workspace_root | String | 远端工作区根目录（如 `/home/openase/workspaces`） |
| agent_cli_path | String | 远端 Agent CLI 可执行文件路径（如 `/usr/local/bin/claude`） |
...
### 25.5 工单绑定：Ticket × Agent × Machine

工单现在绑定到一个三元组：**Workflow（怎么做）+ Agent（谁来做）+ Machine（在哪做）**。

Ticket 表新增字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| target_machine_id | FK (nullable) | 指定执行机器。为空时编排引擎自动选择（匹配 labels）或用 `local` |

分配策略（编排引擎 Tick 中）：

```go
func (s *Scheduler) selectMachine(ctx context.Context, t *ticket.Ticket, wf *workflow.Workflow) (*machine.Machine, error) {
    // 1. 用户手动指定了机器
    if t.TargetMachineID != "" {
        m, err := s.machineRepo.Get(ctx, t.TargetMachineID)
        if m.Status != machine.StatusOnline {
            return nil, fmt.Errorf("machine %s is %s", m.Name, m.Status)
        }
        return m, nil
    }

    // 2. Workflow 有 label 要求（如 coding 不需要 GPU，training 需要 gpu 标签）
    requiredLabels := wf.RequiredMachineLabels  // 如 ["gpu"]
    if len(requiredLabels) == 0 {
        // 无要求，用 local
        return s.machineRepo.GetByName(ctx, "local")
    }

    // 3. 自动匹配：找 online + 标签匹配 + 资源空闲的机器
    candidates, _ := s.machineRepo.ListByLabelsAndStatus(ctx, requiredLabels, machine.StatusOnline)
    // 按资源空闲度排序（CPU 使用率低的优先）
    machine.SortByAvailability(candidates)
    if len(candidates) == 0 {
        return nil, ErrNoAvailableMachine
    }
...
| 字段 | 类型 | 说明 |
|------|------|------|
| required_machine_labels | TEXT[] | 执行该 Workflow 需要的机器标签（如 `{"gpu"}` 表示需要 GPU 机器） |

### 25.6 SSH Agent Runner

当工单分配到远端机器时，Worker 通过 SSH 在远端完成整个工单执行流程：创建工作区 → **远端 git clone** → 写入 Prompt → 启动 Agent CLI。
...
func (w *Worker) runOnRemote(ctx context.Context, m *machine.Machine, t *ticket.Ticket, harness *Harness) error {
    ...
    workDir := fmt.Sprintf("%s/%s", m.WorkspaceRoot, t.Identifier)

    // 1. 创建工作区 + 远端 git clone 所有涉及的 Repo
    ...
    for _, scope := range repoScopes {
        ...
        cloneCmds = append(cloneCmds, fmt.Sprintf(
            `if [ -d "%s/.git" ]; then cd %s && git fetch origin && git checkout -B agent/%s/%s origin/%s; else git clone --depth 50 %s %s && cd %s && git checkout -B agent/%s/%s; fi`,
            repoDir, repoDir, w.agent.Name, t.Identifier, repo.DefaultBranch,
            repo.RepositoryURL, repoDir, repoDir, w.agent.Name, t.Identifier,
        ))
    }
    ...
    cmd := fmt.Sprintf("cd %s && %s -p \"$(cat .harness-prompt.md)\" --output-format stream-json --allowedTools \"Bash,Read,Edit,Write,Glob,Grep\" --max-turns %d",
        workDir, m.AgentCLIPath, harness.MaxTurns)
...
当前机器: gpu-01 (10.0.1.10)
  ...
  工作区: /home/openase/workspaces/ASE-42/
...
hooks:
  on_complete:
    - cmd: "test -f idea-report.md"  # 必须产出报告文件
      on_failure: block
required_machine_labels: []  # 不需要特殊机器
---
...
- 项目描述、状态（planning / active / paused）
...
func (h *HRAdvisor) Recommend(ctx context.Context, project *project.Project) []RoleRecommendation {
    ...
    // 规则 3: 项目从 planning 转 active → 推荐全栈开发者
    if project.Status == "active" && stats.TotalAgents == 0 {
        recs = append(recs, RoleRecommendation{
            Role:   "fullstack-developer",
            Reason: "项目已激活但尚无 Agent。建议招募全栈开发者开始实现。",
        })
    }
...
1. 从角色库复制 Harness 模板到项目的 `.openase/harnesses/` 目录
2. 创建对应的 Workflow 记录（链接到新 Harness）
3. 注册一个新的 Agent 实例（绑定到该 Workflow）
4. 可选：自动创建第一个工单（如测试工程师自动创建"编写现有代码的测试用例"工单）

### 26.5 自定义角色

用户可以创建自己的角色——本质就是写一个 Harness 文件。Web UI 提供角色编辑器：
...
你可以通过 openase CLI 操作平台（已预装在工作区中）：
  ...
  openase project update-status active                     # 更新项目状态
...
| **更新项目状态** | `POST /projects/:id/transition` | MVP 完成后将项目从 planning → active | 需授权 |
...
[产品经理 Agent] 执行工单 ASE-40: "调研竞品并规划 v2.0 功能"
  ...
  5. 推进项目状态：
     → openase project update-status active
```
...
[Harness 优化 Agent] 执行工单 ASE-50: "优化 coding Harness"
  ...
  3. 修改 .openase/harnesses/coding.md：
     - 增加"修改前先运行现有测试"的步骤
     - 调整工作边界规则
  4. 通过 Git 提交 Harness 变更（HarnessWatcher 自动热重载）
  5. 创建验证工单：
...
## 第二十八章 Issue Connector 标准接口

### 28.1 动机

OpenASE 的工单不应该只能手动创建。现实中，需求来自四面八方——GitHub Issues、GitLab Issues、Jira Tickets、Linear Issues、企业内部工单系统、甚至 Slack 消息。如果用户需要在每个系统里手动复制需求到 OpenASE，自动化就断裂了。

Issue Connector 是一个标准化的外部工单源接入机制。它定义了一个统一的接口——任何实现了这个接口的 Connector 都可以将外部 Issue 自动同步到 OpenASE 中成为工单。

### 28.2 架构

```
外部系统                 Connector                    OpenASE
┌──────────┐                                    ┌──────────────┐
│ GitHub   │──pull──→ GitHubConnector ──────→    │              │
│ Issues   │←─push──  (内置)          sync       │  Tickets     │
├──────────┤                                    │              │
│ GitLab   │──pull──→ GitLabConnector ──────→    │  (双向同步     │
│ Issues   │←─push──  (内置)                     │   状态/评论)   │
├──────────┤                                    │              │
│ Jira     │──pull──→ JiraConnector ───────→    │              │
│          │←─push──  (社区插件)                  │              │
├──────────┤                                    │              │
│ 企业内部  │──pull──→ CustomConnector ─────→     │              │
│ 工单系统  │←─push──  (用户自实现)                │              │
├──────────┤                                    │              │
│ Webhook  │──push──→ InboundWebhook ──────→    │              │
│ 任意来源  │          (通用接收器)                 │              │
└──────────┘                                    └──────────────┘
```
...
- **Pull 模式**：OpenASE 定期轮询外部系统，拉取新 Issue / 变更。适合有 API 的系统。
- **Push 模式**：外部系统通过 Webhook 推送事件到 OpenASE。适合支持 Webhook 的系统。
- 大多数 Connector 两种都支持（Pull 兜底 + Push 实时）。

### 28.3 Connector 标准接口

所有 Connector 实现同一个 Go interface：

```go
// domain/connector/interface.go
package connector

// Connector 是外部 Issue 源的标准接口
type Connector interface {
    // 元信息
    ID() string                    // 唯一标识（如 "github", "gitlab", "jira"）
    Name() string                  // 显示名称（如 "GitHub Issues"）

    // Pull 模式：主动拉取
    // since: 上次同步时间，只拉取该时间之后的变更
    PullIssues(ctx context.Context, cfg ConnectorConfig, since time.Time) ([]ExternalIssue, error)

    // Push 模式：解析 Webhook 请求体
    ParseWebhook(ctx context.Context, headers http.Header, body []byte) (*WebhookEvent, error)

    // 回写：将 OpenASE 的状态变更同步回外部系统
    SyncBack(ctx context.Context, cfg ConnectorConfig, update SyncBackUpdate) error

    // 健康检查：验证配置和连接可用性
    HealthCheck(ctx context.Context, cfg ConnectorConfig) error
}

// ExternalIssue 是从外部系统拉取的标准化 Issue
type ExternalIssue struct {
    ExternalID    string            // 外部系统中的唯一 ID（如 "octocat/repo#42"）
    ExternalURL   string            // 外部 Issue 的 URL
    Title         string
    Description   string            // Markdown 格式
    Status        string            // 外部状态（open / closed 等），由 Connector 标准化
    Priority      string            // 标准化为 urgent / high / medium / low，不支持则为空
    Labels        []string
    Author        string
    Assignees     []string
    CreatedAt     time.Time
    UpdatedAt     time.Time
    Comments      []ExternalComment // 最近的评论（可选，用于给 Agent 提供上下文）
    Metadata      map[string]any    // Connector 特定的额外数据
}

// WebhookEvent 是 Push 模式下解析的事件
type WebhookEvent struct {
    Action     string          // created / updated / closed / reopened / commented
    Issue      ExternalIssue
    Comment    *ExternalComment // Action=commented 时有值
}

// SyncBackUpdate 是回写到外部系统的变更
type SyncBackUpdate struct {
    ExternalID    string
    Action        string          // update_status / add_comment / add_label
    Status        string          // Action=update_status 时：映射回外部系统的状态
    Comment       string          // Action=add_comment 时：评论内容
    Label         string          // Action=add_label 时：标签名
}

// ConnectorConfig 是每个 Connector 实例的配置
type ConnectorConfig struct {
    Type          string          // "github" / "gitlab" / "jira" / "custom"
    BaseURL       string          // API 地址（如 "https://api.github.com" 或自建实例地址）
    AuthToken     string          // 认证 Token（从 .env 解引用后的值）
    ProjectRef    string          // 外部项目标识（如 "octocat/hello-world" 或 Jira project key）
    PollInterval  time.Duration   // Pull 模式轮询间隔（默认 5 分钟）
    SyncDirection string          // "pull_only" / "push_only" / "bidirectional"
    Filters       IssueFilters    // 过滤规则（只同步特定标签/状态的 Issue）
    StatusMapping map[string]string // 外部状态 → OpenASE 状态映射
    WebhookSecret string          // Push 模式的 Webhook 签名密钥
}

// IssueFilters 定义哪些 Issue 应该被同步
type IssueFilters struct {
    Labels      []string  // 只同步包含这些标签的 Issue（如 ["openase", "agent-task"]）
    ExcludeLabels []string // 排除包含这些标签的 Issue
    States      []string  // 只同步这些状态的 Issue（如 ["open"]）
    Authors     []string  // 只同步这些作者的 Issue（可选）
}
```

### 28.4 内置 Connector

**GitHub Connector（Phase 2 内置）**

```yaml
# ~/.openase/config.yaml 中配置
connectors:
  - type: github
    ...
    config:
      ...
      webhook_secret: "${GITHUB_WEBHOOK_SECRET}"
      filters:
        labels: ["openase"]                        # 只同步打了 openase 标签的 Issue
        states: ["open"]
      status_mapping:
        open: "backlog"                            # GitHub open → OpenASE backlog
...
| 方向 | 事件 | 行为 |
|------|------|------|
| GitHub → OpenASE | 新 Issue（带 `openase` 标签） | 创建 Ticket（使用项目 is_default 状态），关联 ExternalLink |
| GitHub → OpenASE | Issue 评论更新 | 更新 Ticket 的 ExternalLink 评论缓存，Agent 下次执行能看到 |
| GitHub → OpenASE | Issue 关闭 | Ticket 标记 cancelled（如果还未完成） |
| OpenASE → GitHub | Ticket 状态变为 in_progress | 在 GitHub Issue 上添加评论："Agent 已领取此 Issue" |
| OpenASE → GitHub | Ticket 完成（done） | 关联 PR 链接到 Issue，添加完成评论，关闭 Issue |
| OpenASE → GitHub | Agent 创建了 PR | PR 描述中自动包含 `Fixes #42` 关联 Issue |

**GitLab Connector（Phase 4 内置）**

结构与 GitHub Connector 相同，API 调用替换为 GitLab REST API。`project_ref` 改为 GitLab 的 project ID 或 path。

### 28.5 自定义 Connector：两种接入方式

**方式一：实现 Go 接口（编译时插件）**

适合需要深度集成的企业内部系统。实现 `connector.Connector` 接口后编译进 OpenASE 二进制：

```go
// 企业内部工单系统 Connector 示例
package internal_tracker

type InternalTrackerConnector struct{}

func (c *InternalTrackerConnector) ID() string   { return "internal-tracker" }
func (c *InternalTrackerConnector) Name() string { return "Internal Ticket System" }

func (c *InternalTrackerConnector) PullIssues(ctx context.Context, cfg connector.ConnectorConfig, since time.Time) ([]connector.ExternalIssue, error) {
    // 调用企业内部 API 拉取工单
    resp, err := http.Get(cfg.BaseURL + "/api/tickets?updated_since=" + since.Format(time.RFC3339))
    // ... 解析并映射为 ExternalIssue
}

func (c *InternalTrackerConnector) SyncBack(ctx context.Context, cfg connector.ConnectorConfig, update connector.SyncBackUpdate) error {
    // 将 OpenASE 的状态变更同步回内部系统
}
```
...
```go
connectorRegistry.Register(&internal_tracker.InternalTrackerConnector{})
```

**方式二：Inbound Webhook（零代码接入）**

适合轻量集成或无法修改 OpenASE 源码的场景。外部系统只需要能发 HTTP POST 到 OpenASE 的通用 Webhook 端点：

```
POST /api/v1/connectors/inbound-webhook
...

{
  "action": "created",
  "issue": {
    "external_id": "INTERNAL-123",
    "external_url": "https://tracker.company.com/tickets/123",
    "title": "Fix performance regression in search API",
    "description": "Search latency increased from 50ms to 500ms after...",
    "priority": "high",
...
OpenASE 接收后按照标准流程创建 Ticket，关联 ExternalLink。这样任何能发 HTTP 请求的系统都可以接入——Slack Bot、邮件解析器、CI/CD 系统、甚至一个 cron 脚本。

### 28.6 Connector 管理实体

新增 `IssueConnector` 实体：

| 字段 | 类型 | 说明 |
|------|------|------|
| id | UUID | 主键 |
| project_id | FK | 关联项目 |
| type | String | Connector 类型标识（github / gitlab / jira / inbound-webhook / custom） |
| name | String | 显示名称（如 "acme/backend Issues"） |
| config | JSONB (encrypted) | ConnectorConfig（含 auth_token，加密存储） |
| status | Enum | active / paused / error |
| last_sync_at | DateTime | 上次成功同步时间 |
| last_error | Text | 最后一次错误信息 |
| sync_stats | JSONB | 同步统计（total_synced, last_24h_synced, failed_count） |

### 28.7 同步引擎

编排引擎新增 `ConnectorSyncer` 组件，定期运行 Pull 同步：

```go
// orchestrator/connector_syncer.go
func (s *ConnectorSyncer) syncAll(ctx context.Context) {
    connectors, _ := s.connectorRepo.ListActive(ctx)

    for _, conn := range connectors {
        impl := s.registry.Get(conn.Type)  // 获取 Connector 实现

        issues, err := impl.PullIssues(ctx, conn.Config, conn.LastSyncAt)
        if err != nil {
            conn.Status = "error"
            conn.LastError = err.Error()
            s.connectorRepo.Save(ctx, conn)
            continue
        }

        for _, issue := range issues {
            s.syncIssueToTicket(ctx, conn, issue)
        }
...
func (s *ConnectorSyncer) syncIssueToTicket(ctx context.Context, conn *IssueConnector, issue ExternalIssue) {
    // 1. 检查是否已同步过（通过 ExternalLink 查重）
    existing, _ := s.linkRepo.FindByExternalID(ctx, issue.ExternalID)
    if existing != nil {
        // 已存在 → 更新（标题、描述、状态变更）
        s.updateExisting(ctx, existing, issue)
        return
    }
    ...
    ticket := &ticket.Ticket{
        ...
        Status:      mapStatus(conn.Config.StatusMapping, issue.Status),
        Priority:    mapPriority(issue.Priority),
        ExternalRef: issue.ExternalID,
    }

    // 自动分配 Workflow（如果配置了 auto_workflow）
    if conn.Config.AutoWorkflow != "" {
        ticket.WorkflowID = conn.Config.AutoWorkflow
    }

    s.ticketRepo.Save(ctx, ticket)

    // 3. 创建 ExternalLink
    s.linkRepo.Save(ctx, &TicketExternalLink{
        TicketID:   ticket.ID,
        LinkType:   conn.Type + "_issue",
...
### 28.8 状态回写（Bidirectional Sync）

当 OpenASE 工单状态变更时，通过 Connector 的 `SyncBack` 方法回写到外部系统：

| OpenASE 事件 | 回写操作 | 外部系统表现 |
|-------------|---------|------------|
| Agent 领取工单 | `add_comment` | GitHub Issue 评论："🤖 Agent claude-01 已领取此 Issue，正在处理中..." |
| Agent 提交 PR | `add_comment` | GitHub Issue 评论："📝 Agent 已提交 PR: https://..." |
| on_complete Hook 通过 | `add_comment` | GitHub Issue 评论："✅ 所有质量检查通过，等待审批" |
| 工单完成（done） | `update_status` + `add_comment` | 关闭 GitHub Issue，评论："🎉 已完成，PR 已合并" |
| 工单出错 | `add_comment` + `add_label` | 评论错误原因，添加 `openase-retrying` 标签 |

### 28.9 API 端点

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/projects/:projectId/connectors` | 列出项目的 Connector |
| POST | `/api/v1/projects/:projectId/connectors` | 添加 Connector |
| PATCH | `/api/v1/connectors/:connectorId` | 更新 Connector 配置 |
| DELETE | `/api/v1/connectors/:connectorId` | 删除 Connector |
| POST | `/api/v1/connectors/:connectorId/test` | 测试连接（调用 HealthCheck） |
| POST | `/api/v1/connectors/:connectorId/sync` | 手动触发一次 Pull 同步 |
| GET | `/api/v1/connectors/:connectorId/stats` | 同步统计 |
| POST | `/api/v1/connectors/inbound-webhook` | 通用 Inbound Webhook 接收端点 |
...
```
┌──────────────────────────────────────────────────────┐
│ Issue Connectors                        [添加 Connector] │
├──────────────────────────────────────────────────────┤
│                                                      │
│ ● GitHub: acme/backend                               │
│   双向同步 · 上次同步: 2 分钟前 · 已同步 47 个 Issue      │
│   过滤: label=openase · 状态: 活跃                     │
│   [同步一次]  [暂停]  [配置]                            │
│                                                      │
│ ● GitHub: acme/frontend                              │
│   双向同步 · 上次同步: 2 分钟前 · 已同步 23 个 Issue      │
│   过滤: label=openase · 状态: 活跃                     │
│   [同步一次]  [暂停]  [配置]                            │
│                                                      │
│ ○ Inbound Webhook: 内部工单系统                        │
│   仅接收 · 已接收 12 个 Issue · 状态: 活跃               │
│   端点: /api/v1/connectors/inbound-webhook             │
│   [查看日志]  [暂停]  [配置]                            │
│                                                      │
└──────────────────────────────────────────────────────┘
```
...
| 组件 | 变化 |
|------|------|
| Domain | 新增 `domain/connector/` 子包（interface、entity、config） |
| Infrastructure | 新增 `infra/connector/github/`、`infra/connector/gitlab/`、`infra/connector/webhook/` |
| Orchestrator | 新增 `ConnectorSyncer`，由 robfig/cron 定期触发 Pull 同步 |
| API | 新增 Connector CRUD 端点 + Inbound Webhook 端点 |
| Application | 新增 `SyncIssueCmd`、`SyncBackCmd` |
| 数据库 | 新增 `issue_connectors` 表 |
...
### 29.2 设计：纯自定义状态 + Workflow Pickup 驱动

**没有 Phase 层，没有硬编码状态机。** Custom Status 是唯一的状态维度，用户完全自由定义。编排引擎通过 Workflow 的 `pickup / finish / fail` 配置知道该关心哪些状态。
...
**每个列本身没有语义——语义完全由 Workflow 的 pickup/finish 配置赋予。** 同一个"待测试"列，对 test Workflow 来说是 pickup（入口），对 coding Workflow 来说可能是 finish（出口）。
...
```
pickup_status_id   FK → TicketStatus.id   -- 编排引擎扫描这个状态的工单
finish_status_id   FK → TicketStatus.id   -- Agent 完成后移到这个状态
```

编排引擎查询变为：`WHERE status_id = workflow.pickup_status_id AND assigned_agent_id IS NULL AND retry_paused = false AND (next_retry_at IS NULL OR next_retry_at <= now())`。没有 fail 状态，没有状态机，Agent 出错就退避重试。

### 29.4 默认状态模板

每个新项目创建时自动生成一组默认的 Custom Status，用户可以直接使用、修改或删除后完全自定义：

| 默认 Custom Status | 颜色 | 说明 | is_default |
|-------------------|------|------|-----------|
| Backlog | 灰色 | 积压 | ✅ |
| Todo | 蓝色 | 就绪等待 | |
| In Progress | 黄色 | 进行中 | |
...
默认的 coding Workflow 自动配置 pickup: "Todo", finish: "Done"。用户修改看板列后只需要对应修改 Workflow 的 pickup/finish 配置即可。

### 29.5 状态转换规则

**用户自由操作**——任何 Custom Status 之间的手动拖拽都允许，没有状态机阻拦。用户想把工单从"已完成"拖回"进行中"？可以。从"Idea"直接跳到"已投稿"？也可以。OpenASE 不判断合理性——那是团队自己的流程规范。

**编排引擎自动触发**——只在两个时刻自动改变 status_id：

1. **Agent 完成** → `status_id = workflow.finish_status_id`
2. **Agent 出错** → 清空 assigned_agent_id，指数退避后重试（不存在 fail 状态）
...
│ backlog  │unstarted │ started  │ started  │ started  │ completed   │
│ (3)      │ (2)      │ (1)      │ (2)      │ (0)      │ (5)         │
├──────────┼──────────┼──────────┼──────────┼──────────┼─────────────┤
...
Phase 分隔线在看板上用微妙的视觉分组表示（如不同 Phase 的列之间有一条细灰线），让用户知道哪些列属于同一个阶段。
...
Agent 在 Prompt 中看到有语义的 Custom Status 名称（"文献调研"比"in_progress"有用得多），编排引擎通过 `pickup_status_id` 匹配而非状态名。

### 29.8 Issue Connector 的状态映射

Connector 的 `status_mapping` 直接映射到 Custom Status 名称：

```yaml
status_mapping:
  open: "Idea"            # GitHub open → 项目中 name="Idea" 的 TicketStatus
  in_progress: "实验中"
  closed: "已录用"
```

系统通过 Custom Status 名称查找对应的 `status_id`。如果名称不存在则使用项目的 `is_default=true` 状态。

### 29.9 API 端点

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/projects/:projectId/statuses` | 列出项目的所有 Custom Status（按 Phase 分组） |
| POST | `/api/v1/projects/:projectId/statuses` | 创建 Custom Status `{name, phase, color, icon, position}` |
| PATCH | `/api/v1/statuses/:statusId` | 更新 Custom Status（名称、颜色、图标、排序） |
| DELETE | `/api/v1/statuses/:statusId` | 删除 Custom Status（该状态下的工单迁移到同 Phase 的默认状态） |
...
```sql
-- 编排引擎按 status_id + assigned_agent_id 查询
-- 每个 Workflow 的 pickup_status_id 不同，所以不在索引中固化具体值
CREATE INDEX idx_tickets_dispatch ON tickets (project_id, status_id, assigned_agent_id, priority, created_at);
...
| 章节 | 影响 |
|------|------|
| 第六章 领域模型 | Ticket.status 改为 status_id FK；新增 TicketStatus 实体；Workflow 新增 pickup_status_id / finish_status_id |
| 第七章 已重写 | 不再有状态机，编排引擎只看 Workflow 的 pickup_status_id |
| 第八章 Hook | 触发点不再引用具体状态名，只关心编排生命周期事件 |
| 第十章 编排引擎 | 调度查询改为 `WHERE status_id = workflow.pickup_status_id AND assigned_agent_id IS NULL` |
| 第十六章 伪代码 | 状态判断改为 `ticket.StatusID == wf.PickupStatusID` |
| 第十八章 API | Ticket 响应体包含 `status: {id, name, color, icon}` 嵌套对象 |
| 第二十章 数据库索引 | 索引字段从 status enum 改为 status_id FK |
| 第二十八章 Connector | status_mapping 目标为 Custom Status 名称 |

核心简化：**没有状态机了。** 编排引擎只匹配 `status_id == pickup_status_id`，不做任何状态转换合法性校验。状态流转完全由用户（看板拖拽）和 Agent（完成时移到 finish 状态）驱动。出错不改状态，只是退避重试。
...
| `project.status` | string | 项目状态 | `active` |
| `project.default_branch` | string | 主仓库默认分支 | `main` |
...
| `repos[].path` | string | 工作区中的本地路径 | `/workspaces/ASE-42/backend` |
| `repos[].branch` | string | 当前工作分支 | `agent/claude-01/ASE-42` |
| `repos[].default_branch` | string | 默认分支 | `main` |
| `repos[].labels` | list | 仓库标签 | `["go", "backend", "api"]` |
| `repos[].is_primary` | bool | 是否主仓库 | `true` |
...
| `agent.capabilities` | list | 能力标签 | `["backend", "go", "testing"]` |
...
| `machine.workspace_root` | string | 远端工作区根目录 | `/home/openase/workspaces` |
...
| `workspace` | string | 工作区根路径 | `/workspaces/ASE-42` |
...
| `workflow.pickup_status` | string | Pickup 状态名 | `Todo` |
| `workflow.finish_status` | string | Finish 状态名 | `Done` |
...
| `selectattr(attr, value)` | 按属性过滤列表 | `{{ repos \| selectattr("is_primary", true) }}` |
...
## 第三十一章 内嵌 AI 助手（Ephemeral Chat）

### 31.1 定位

OpenASE 的核心是工单驱动的编排系统——Agent 领取工单、按 Harness 执行、走 Hook 验证。但日常使用中有大量**不需要工单**的轻量 AI 交互需求：

- 编辑 Harness 时让 AI 帮忙写/改 Prompt
- 在看板旁边问"这个项目目前进展如何"
- 让 AI 帮忙拆分一个大需求为多个子工单
- 问"ASE-42 为什么失败了"然后让 AI 分析日志
- 让 AI 帮忙配置定时任务的 cron 表达式

这些场景的共同点：**临时对话、不绑定工单、不走编排引擎、不触发 Hook、用完即走。**
...
│                                                        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │ Harness 编辑器 │  │ 看板侧边栏    │  │ 工单详情页    │  │
│  │ "帮我优化这段  │  │ "项目进展如何" │  │ "为什么失败了"│  │
│  │  Prompt"      │  │              │  │              │  │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  │
│         │                 │                 │          │
│         └─────────────────┼─────────────────┘          │
...
Ephemeral Chat 的价值在于它**知道项目上下文**。后端在调用 Agent CLI 之前，自动将相关信息注入到 system prompt 中：

```go
func (h *EphemeralChatHandler) buildSystemPrompt(ctx context.Context, req ChatRequest) string {
    var sb strings.Builder

    sb.WriteString("你是 OpenASE 平台的 AI 助手。以下是当前上下文：\n\n")

    // 根据触发来源注入不同的上下文
    switch req.Source {

    case "harness_editor":
        // 注入当前 Harness 内容 + 变量字典
        wf, _ := h.workflowRepo.Get(ctx, req.WorkflowID)
        sb.WriteString("## 当前正在编辑的 Harness\n\n")
        sb.WriteString("```markdown\n" + wf.HarnessContent + "\n```\n\n")
        sb.WriteString("## 可用模板变量\n\n")
        sb.WriteString(h.variableDict.ToMarkdown())

    case "project_sidebar":
        // 注入项目概况、工单统计、最近活动
        project, _ := h.projectRepo.Get(ctx, req.ProjectID)
        stats := h.statsQuery.GetProjectStats(ctx, req.ProjectID)
        sb.WriteString(fmt.Sprintf("## 项目: %s\n\n%s\n\n", project.Name, project.Description))
        sb.WriteString(fmt.Sprintf("## 工单统计\n\n- 总数: %d\n- 进行中: %d\n- 已完成: %d\n- 失败: %d\n\n",
            stats.Total, stats.InProgress, stats.Completed, stats.Retrying))
        recentActivity, _ := h.activityRepo.ListRecent(ctx, req.ProjectID, 20)
        sb.WriteString("## 最近活动\n\n")
        for _, a := range recentActivity {
            sb.WriteString(fmt.Sprintf("- [%s] %s\n", a.CreatedAt.Format("01/02 15:04"), a.Message))
        }

    case "ticket_detail":
        // 注入工单完整信息、活动日志、Hook 执行历史
        ticket, _ := h.ticketRepo.Get(ctx, req.TicketID)
        sb.WriteString(fmt.Sprintf("## 工单: %s — %s\n\n", ticket.Identifier, ticket.Title))
        sb.WriteString(fmt.Sprintf("状态: %s | 优先级: %s | 尝试: %d/%d\n\n",
            ticket.StatusName, ticket.Priority, ticket.AttemptCount, ticket.MaxAttempts))
        if ticket.Description != "" {
            sb.WriteString("### 描述\n\n" + ticket.Description + "\n\n")
        }
        activities, _ := h.activityRepo.ListByTicket(ctx, ticket.ID, 30)
        sb.WriteString("### 活动日志（最近 30 条）\n\n")
        for _, a := range activities {
            sb.WriteString(fmt.Sprintf("- [%s] %s\n", a.CreatedAt.Format("15:04:05"), a.Message))
        }
    }

    // 通用：平台操作能力说明
    sb.WriteString("\n## 你可以帮用户做的事\n\n")
    sb.WriteString("- 编写和优化 Harness Prompt\n")
    sb.WriteString("- 分析项目进展和工单状态\n")
    sb.WriteString("- 拆分需求为子工单\n")
    sb.WriteString("- 解释 Agent 执行日志和 Hook 失败原因\n")
    sb.WriteString("- 建议 cron 表达式\n")
    sb.WriteString("- 推荐应该招募哪些角色\n")
...
Ephemeral Chat 可以帮用户执行平台操作，但**需要用户确认**——AI 生成操作意图，前端展示确认弹窗，用户点确认后才真正执行。

```
用户: 帮我把 ASE-42 拆成 3 个子工单：注册 API、登录 API、权限模型

AI: 好的，我建议创建以下 3 个子工单：

  1. ASE-43: 实现用户注册 API (coding, high)
  2. ASE-44: 实现用户登录 API (coding, high)
  3. ASE-45: 实现 RBAC 权限模型 (coding, medium)

  全部设为 ASE-42 的 sub-issue，workflow 为 coding。

  [确认创建 3 个工单]  [修改]  [取消]

用户: (点击确认)

AI: ✅ 已创建 ASE-43、ASE-44、ASE-45，均关联为 ASE-42 的子工单。
```

实现方式：AI 的响应中包含结构化的 action block，前端识别并渲染为确认按钮：

```json
// AI 响应中的 stream-json 事件
{
...
前端收到 `action_proposal` → 渲染确认 UI → 用户确认 → 前端逐个调用 API 执行 → 回显结果。**AI 不直接执行操作**，一定经过用户确认。

### 31.5 前端触发入口

| 入口位置 | 触发方式 | 注入的上下文 | 典型问题 |
|---------|---------|------------|---------|
| Harness 编辑器侧栏 | 编辑器右侧"AI 助手"Tab | 当前 Harness 内容 + 变量字典 | "帮我优化工作边界的描述"、"加一个重试时的续接提示" |
| 看板页右侧抽屉 | 看板顶部"问 AI"按钮 | 项目信息 + 工单统计 + 最近活动 | "项目进展如何"、"哪些工单卡住了"、"推荐我招什么角色" |
| 工单详情页底部 | 详情页"AI 分析"按钮 | 工单完整信息 + 活动日志 + Hook 历史 | "为什么失败了"、"帮我拆成子工单"、"Agent 做了什么" |
...
│                                      │                               │
│  ---                                 │ ┌───────────────────────────┐ │
│  status:                             │ │ 帮我改进"工作边界"这一节，  │ │
│    pickup: "Todo"                    │ │ 增加对测试文件的保护规则    │ │
│  ---                                 │ └───────────────┬───────────┘ │
│                                      │                 │ [发送]       │
│  # Coding Workflow                   │                               │
│                                      │ AI: 好的，我建议将"工作边界"  │
...
"应用到编辑器"点击后：AI 输出的 diff 直接 patch 到编辑器内容中，用户可以 undo。

### 31.7 会话管理

| 特性 | 设计 |
|------|------|
| 会话生命周期 | 用户关闭侧栏 / 切换页面 → 会话结束，Agent CLI 进程退出 |
| 会话上下文 | 支持多轮对话（通过 `--resume session_id` 续接） |
| 历史记录 | 不持久化。临时对话不存数据库，关闭即丢弃 |
| 并发 | 每个用户同时最多 1 个 Ephemeral Chat 会话，新会话自动关闭旧会话 |
| 成本控制 | 单次会话最大 10 轮对话 / $2.00 预算，超限提示"请创建工单让 Agent 处理" |

### 31.8 API 端点

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/chat` | 发起 Ephemeral Chat，SSE 流式返回 |
| DELETE | `/api/v1/chat/:sessionId` | 关闭对话（释放 Agent CLI 进程） |
...
首次请求 `session_id` 为 null → 创建新会话；后续请求带上返回的 `session_id` → 续接对话。

响应：SSE 流，每行一个 JSON 事件：

```
event: message
data: {"type": "text", "content": "好的，我建议将..."}

event: message
data: {"type": "diff", "file": "harness content", "hunks": [...]}

event: message
data: {"type": "action_proposal", "actions": [...], "summary": "创建 3 个子工单"}
...
| 生命周期 | 工单从 pickup 到 finish | 用户打开到关闭侧栏 |
| 上下文来源 | Harness Prompt + 工单描述 | 后端注入的页面上下文 |
| 状态管理 | 工单状态流转、attempt_count | 无状态，用完即走 |
| Hook | on_claim / on_complete / on_done | 无 |
| 成本追踪 | 记入工单 cost_amount | 记入项目级 ephemeral_chat_cost |
| 平台操作 | Agent Token 直接执行 | 用户确认后才执行 |
| 持久化 | ActivityEvent 全量记录 | 不持久化（或仅记入项目级日志） |
| 并发 | 多个 Agent 并行执行多工单 | 每用户最多 1 个会话 |
...
4. **选择机器**（可选）：如果工单需要特殊资源（GPU 训练、大内存处理），指定机器
...
# 分配给特定机器
openase ticket update ASE-{{ ticket.identifier }} --status "待开发" --machine gpu-01
...
| `project.workflows[].current_active` | int | 当前正在执行的工单数 |
...
| `project.machines[].resources` | object | 资源快照（CPU/内存/GPU） |
...
**可迭代**：Dispatcher 的 Harness 像其他角色一样存在 Git 仓库里，可以 Diff、Review、Rollback。refine-harness 元工作流可以分析 Dispatcher 的分配历史，优化分配策略。
...
**通用 Webhook 是万能出口**——就像 Issue Connector 的 Inbound Webhook 是万能入口一样。任何能接收 HTTP POST 的系统都能作为通知渠道。
...
```json
// 只在工单进入"待开发"列时通知
{"event_type": "ticket.status_changed", "filter": {"new_status": "待开发"}}
...
| 事件类型 | 触发时机 | 默认消息模板 |
|---------|---------|------------|
| `ticket.created` | 工单创建 | "📋 新工单 {{ ticket.identifier }}: {{ ticket.title }}" |
| `ticket.status_changed` | 工单状态变更 | "🔄 {{ ticket.identifier }} 状态变为 {{ new_status }}" |
| `ticket.assigned` | Agent 领取工单 | "🤖 {{ agent.name }} 领取了 {{ ticket.identifier }}" |
| `ticket.completed` | 工单完成（到达 finish 状态） | "✅ {{ ticket.identifier }} 已完成" |
| `ticket.error` | Agent 执行出错（即将退避重试） | "🔄 {{ ticket.identifier }} 出错: {{ reason }}，退避后重试" |
| `ticket.cancelled` | 工单取消 | "🚫 {{ ticket.identifier }} 已取消" |
| `ticket.stalled` | Agent Stall | "⚠️ {{ ticket.identifier }} 的 Agent 无响应" |
| `ticket.error_rate_high` | 连续 3+ 次失败 | "🔴 {{ ticket.identifier }} 连续失败 {{ consecutive_errors }} 次，退避 {{ backoff }} 后重试" |
| `ticket.budget_exhausted` | 预算耗尽，重试暂停 | "💰 {{ ticket.identifier }} 预算耗尽（${{ cost_amount }}/${{ budget_usd }}），已暂停重试" |
...
| `machine.degraded` | 机器资源告警 | "⚠️ {{ machine.name }} 磁盘剩余 {{ disk_free_gb }}GB" |
| `connector.sync_error` | Connector 同步失败 | "⚠️ {{ connector.name }} 同步失败: {{ error }}" |
...
Skill 是 Agent CLI（Claude Code、Codex 等）原生支持的能力扩展单元。每个 Skill 是一个 Markdown 文件（`SKILL.md`），定义了一组可复用的指令、流程或工具调用方式。例如：
...
Skill 对 Agent 来说就是"你会什么"。Harness 是"你的角色是什么"。**Harness 绑定 Skills = 这个角色拥有哪些能力。**
...
│                                                         │
│ 绑定的 Skills:                                           │
│   ├── openase-platform   (操作平台：创建工单、注册 Repo)    │
│   ├── commit             (Git commit 规范)               │
│   ├── review-code        (代码审查规范)                    │
│   └── deploy-docker      (Docker 部署流程)                │
│                                                         │
│ Agent 执行时：                                            │
│   1. 编排引擎将绑定的 Skills 复制到工作区的 agent skills 目录│
│   2. Agent CLI 自动加载 skills 目录中的所有 SKILL.md       │
│   3. Agent 在执行过程中可以调用这些 Skills                  │
│   4. Agent 也可以自己创建新的 Skill                        │
└─────────────────────────────────────────────────────────┘
```

### 34.3 存储结构

Skills 存储在项目的 `.openase/skills/` 目录中，跟 Harness 和 Hook 一起 Git 版本控制：
...
│       │   └── SKILL.md          # 平台操作能力
│       ├── commit/
│       │   └── SKILL.md          # Git commit 规范
│       ├── review-code/
│       │   └── SKILL.md          # 代码审查规范
│       ├── deploy-docker/
│       │   └── SKILL.md          # Docker 部署
│       └── python-best-practices/
│           └── SKILL.md          # Python 最佳实践
```

### 34.4 Skill 与 Harness 的绑定

Harness 的 YAML Frontmatter 中声明该角色使用哪些 Skills：
...
status:
  ...
  finish: "Done"
skills:
  - openase-platform       # 平台操作能力（几乎所有角色都需要）
  - commit                 # Git 规范
  - review-code            # 代码审查
  # - deploy-docker        # 注释掉 = 该角色不需要部署能力
---
```

### 34.5 编排引擎的 Skill 注入流程
...
func (w *Worker) injectSkills(ctx context.Context, workspace string, harness *Harness, adapterType string) error {
    ...
    case "codex-app-server":
        skillsDir = filepath.Join(workspace, ".codex", "skills")
    default:
        skillsDir = filepath.Join(workspace, ".agent", "skills")
    }
    os.MkdirAll(skillsDir, 0755)

    // 2. 将 Harness 绑定的 Skills 从 .openase/skills/ 复制到 Agent skills 目录
    for _, skillName := range harness.Skills {
        src := filepath.Join(w.projectSkillsDir, skillName)
        dst := filepath.Join(skillsDir, skillName)

        // fail fast：Skill 目录不存在或 SKILL.md 格式错误 → 跳过，记警告
        if err := w.validateAndCopy(src, dst); err != nil {
            w.logger.Warn("skill injection skipped",
                "skill", skillName, "err", err)
            w.metrics.Counter("openase.skill.injection_skipped",
                provider.Tags{"skill": skillName, "reason": err.Error()}).Inc()
            continue
        }
    }

    // 3. 始终注入 openase-platform skill（除非 Harness 显式排除）
    if !harness.ExcludesPlatformSkill() {
        w.validateAndCopy(
            filepath.Join(w.builtinSkillsDir, "openase-platform"),
            filepath.Join(skillsDir, "openase-platform"),
        )
    }
...
这是最重要的 Skill——它教 Agent 如何操作 OpenASE 平台。是 Agent 自治闭环（第二十七章）的基础能力。

```markdown
# OpenASE Platform Operations

你可以通过以下命令操作 OpenASE 平台。所有操作都需要平台授权（环境变量中已注入 Token）。

## 工单操作

### 创建工单
openase ticket create --title "标题" --priority high --workflow coding --parent ASE-42

### 更新当前工单
openase ticket update $OPENASE_TICKET_ID --description "补充描述"

### 更改工单状态（分配到其他角色）
openase ticket update $OPENASE_TICKET_ID --status "待测试"

### 关联外部链接
openase ticket link $OPENASE_TICKET_ID --url "https://github.com/..." --type github_issue

### 查询工单
openase ticket list --project $OPENASE_PROJECT_ID --status "进行中" --limit 10

## 项目操作

### 注册新仓库
openase project add-repo --name "new-service" --url "https://github.com/..." --labels "go,backend"

### 更新项目描述
openase project update $OPENASE_PROJECT_ID --description "新的描述"

## 定时任务

### 创建定时任务
openase scheduled-job create --name "weekly-scan" --cron "0 9 * * 1" --workflow security

## Skill 管理

### 刷新 Skills（不退出当前会话，重新加载 skills 目录）
openase skill refresh
...
Agent 在执行工单过程中可以自己创建新的 Skill——比如一个 coding Agent 在处理某类任务时总结出了一套模式，可以写成 Skill 供下次使用。
...
Agent 执行工单 ASE-42
    │
    ├── 发现重复模式：每次部署 Docker 都要执行同一套步骤
    │
    ├── 创建 Skill 文件：
    │   写入 .claude/skills/deploy-docker/SKILL.md
    │   （在 Agent CLI 的 skills 目录中）
    │
    └── 工单完成
            │
            ▼
    编排引擎 on_done 阶段 — Skill 收割（harvest）
    │
    ├── 扫描 Agent CLI skills 目录中的新文件
    │   （对比 Harness 绑定的 Skills 列表，找出新增的）
    │
    ├── 验证格式（fail fast：SKILL.md 不存在或格式错误 → 跳过）
    │
    ├── 复制到项目的 .openase/skills/ 目录
    │
    ├── Git commit：
    │   "feat(skills): auto-harvest deploy-docker from ASE-42"
    │
    └── 自动绑定到当前 Harness：
        更新 harness YAML 的 skills 列表，追加 deploy-docker
```
...
func (w *Worker) harvestNewSkills(ctx context.Context, workspace string, harness *Harness, adapterType string) {
    ...
    // 获取当前 skills 目录下的所有 skill
    currentSkills, _ := listSkillDirs(skillsDir)

    // 对比 Harness 绑定的 Skills，找出新增的
    boundSkills := harness.SkillSet()
    for _, skillName := range currentSkills {
        if boundSkills.Contains(skillName) {
            continue  // 已绑定的，跳过
        }
        ...
        // fail fast 验证
        if err := validateSkillFormat(skillPath); err != nil {
            w.logger.Warn("harvested skill invalid, skipping",
                "skill", skillName, "err", err)
            continue
        }

        // 复制到项目 skills 库
        dst := filepath.Join(w.projectSkillsDir, skillName)
        copyDir(filepath.Join(skillsDir, skillName), dst)

        // 记录活动
        w.eventBus.Publish(ctx, "skill.events", SkillHarvestedEvent{
            SkillName:   skillName,
            HarvestFrom: harness.Name,
            TicketID:    w.ticket.ID,
        })

        w.logger.Info("harvested new skill", "skill", skillName, "from_ticket", w.ticket.Identifier)
    }
}
```

### 34.8 Agent 运行中刷新 Skill

Agent 不需要等工单完成才能使用新 Skill。通过 Platform API 调用 `openase skill refresh`，编排引擎会：

1. 重新扫描 `.openase/skills/` 目录
2. 将新增的 Skill 复制到当前工作区的 Agent CLI skills 目录
3. Agent CLI 下次调用 Skill 时自动加载新文件
...
# 3. 还可以给其他 Harness 绑定这个 Skill
openase skill bind my-new-skill --harness testing
```

### 34.9 Skill 实体与管理

**Skill 实体（数据库索引，内容在 Git 中）：**

| 字段 | 类型 | 说明 |
|------|------|------|
| id | UUID | 主键 |
| project_id | FK | 所属项目 |
| name | String | Skill 名称（目录名），项目内唯一 |
| description | String | Skill 描述（从 SKILL.md 的第一行 `#` 标题提取） |
| path | String | 在 Git 仓库中的路径（如 `.openase/skills/commit/SKILL.md`） |
| is_builtin | Boolean | 是否为内置 Skill（如 openase-platform） |
| is_enabled | Boolean | 是否启用（禁用后不会被注入到任何 Agent） |
| created_by | String | 创建者（`user:gary` 或 `agent:claude-01 via ASE-42`） |
| bound_harnesses | TEXT[] | 绑定到哪些 Harness（冗余字段，加速查询） |
...
| 操作 | 说明 |
|------|------|
| 创建 | 手动在 `.openase/skills/` 下创建目录 + SKILL.md，或 Agent 自动 harvest |
| 绑定 | 在 Harness YAML 的 `skills` 列表中添加 skill 名称 |
| 解绑 | 从 Harness YAML 中移除 |
| 禁用 | `openase skill disable <name>`——Skill 文件保留，但不会被注入到任何 Agent |
| 启用 | `openase skill enable <name>` |
| 删除 | `openase skill delete <name>`——从 `.openase/skills/` 目录删除 + 从所有 Harness 中解绑 |
| 刷新 | `openase skill refresh`——重新扫描目录，同步数据库索引 |

### 34.10 内置 Skill 库

OpenASE 自带一组内置 Skills，在 `openase init` 时复制到项目的 `.openase/skills/` 中：

| Skill | 说明 | 默认绑定到 |
|-------|------|-----------|
| `openase-platform` | 平台操作能力（创建工单、注册 Repo 等） | 所有 Harness |
...
| POST | `/api/v1/projects/:projectId/skills` | 创建 Skill `{name, content}` |
| GET | `/api/v1/skills/:skillId` | 获取 Skill 详情（含 SKILL.md 内容） |
| PUT | `/api/v1/skills/:skillId` | 更新 Skill 内容 |
...
| POST | `/api/v1/skills/:skillId/bind` | 绑定到 Harness `{harness_ids: [...]}` |
| POST | `/api/v1/skills/:skillId/unbind` | 从 Harness 解绑 `{harness_ids: [...]}` |
| POST | `/api/v1/projects/:projectId/skills/refresh` | 重新扫描 skills 目录，同步 DB 索引 |

### 34.13 Skill 校验规则（Fail Fast）

加载 Skill 时严格校验，格式错误的直接跳过，不影响其他 Skill 和 Agent 正常运行：

| 检查项 | 失败行为 |
|--------|---------|
| `SKILL.md` 文件不存在 | 跳过该 Skill，日志 warning |
| `SKILL.md` 文件为空 | 跳过，warning |
| 文件大小 > 100KB | 跳过，warning（防止注入巨量 Prompt） |
| 目录名包含特殊字符 | 跳过，warning |
| YAML Frontmatter 语法错误（如果有） | 跳过 Frontmatter，只用 Markdown 正文 |

### 34.14 对架构的影响

| 组件 | 变化 |
|------|------|
| Domain | 新增 `domain/skill/`（entity、repository、service） |
| Orchestrator Worker | 启动 Agent 前执行 Skill 注入；on_done 后执行 Skill 收割 |
| Infrastructure | 新增 `infra/skill/`（文件系统操作、Git commit、格式校验） |
| API | 新增 Skill CRUD + bind/unbind 端点 |
| CLI | 新增 `openase skill` 子命令（list/create/enable/disable/refresh/bind） |
| Web UI | 设置中新增 Skill 管理页面 |
| 数据库 | 新增 `skills` 表 |
| Harness YAML | 新增 `skills` 字段（String 数组） |
| Git | Skills 变更（创建、修改、harvest）产生 Git commit |
...
```
    ...
    ▼            ▼            ▼
  手动创建     内置复制     Agent 自创
  (Web UI/CLI) (openase init) (harvest)
    │            │            │
    └────────────┼────────────┘
                 ▼
         .openase/skills/xxx/SKILL.md
         (Git 版本控制)
                 │
          ┌──────┼──────┐
          ▼      ▼      ▼
...
5. 编码助手接手 (pickup: Todo)
   ...
   → 创建 feature branch: agent/claude-01/ASE-42
...
有一个微小 GAP：**PR 打回后自动重新进入 pickup 的机制**。当前设计中，人类拖回 "Todo" 后，编码助手的 pickup 是 "Todo"，下个 Tick 会重新领取——但此时 `assigned_agent_id` 可能还没清空（上次的 Agent 会话已结束但字段可能没重置）。

修复：Worker 在 Agent 完成（finish 或 fail）后必须清空 `assigned_agent_id`。工单被人类手动拖回 pickup 列时也应该清空。这个在第七章 7.4 节已有设计（`t.AssignedAgentID = ""`），但需要确保人类手动拖拽时也触发清空。

→ 需要补充：**任何 status_id 变更（无论是 Agent 自动还是人类手动）都清空 `assigned_agent_id`。** 这保证工单回到任何 pickup 列都能被重新领取。
...
| 角色 | pickup | finish | 机器标签要求 | 说明 |
|------|--------|--------|------------|------|
| Idea 生产助手 | — | — | — | 定时任务触发，产出到 "Idea Pool" |
| Dispatcher | Idea Pool | — | — | 评估 Idea 可行性，分到"待调研"或直接"待实验" |
| 文献调研助手 | 待调研 | 待实验 | — | 搜索论文、分析可行性 |
| 实验验证助手 | 待实验 | Success 或 Fail | gpu | 编写实验代码、跑实验 |
| 报告编写助手 | 待报告 | 人类审核 | — | 编写精美报告 |
...
4. 实验验证助手接手 (pickup: "待实验")
   → Workflow required_machine_labels: ["gpu"]
   → 编排引擎自动匹配 gpu-01 或 gpu-02
   → SSH 到 GPU 机器启动 Claude Code
   → 编写实验代码、跑训练
...
一个小 GAP：**实验验证助手需要根据实验结果动态决定 finish 状态是 "Success" 还是 "Fail"。** 当前 Harness 的 `status.finish` 是一个固定值。需要支持 Agent 通过 Platform API 自己选择目标状态，而不是编排引擎固定移到 finish。

修复方案：Harness 中 `status.finish` 可以设为空。当 finish 为空时，编排引擎不自动移状态——由 Agent 自己通过 `openase ticket update --status "Success"` 或 `--status "Fail"` 决定。编排引擎只负责清空 `assigned_agent_id`。

→ 需要补充到第七章：**`status.finish` 为空时，Agent 自己决定目标状态。** 这是"Agent 自治"的自然延伸。
...
| GAP | 影响场景 | 修复方案 | 涉及章节 |
|-----|---------|---------|---------|
| 人类手动拖拽工单时需清空 assigned_agent_id | 场景二（PR 打回重新领取） | 任何 status_id 变更都清空 assigned_agent_id | 第七章 7.4 |
...
Layer 1 (核心 CRUD，依赖 Layer 0)
  ...
  F09 Workflow + Harness Git 存储 → F04
...
Layer 5 (Git 集成，依赖 Layer 2)
  ...
  F34 GitHub Webhook 接收 (PR 事件) → F08
  F35 PR 状态同步到 TicketRepoScope → F34
  F36 全部 PR merged → 触发 finish → F35, F17
...
Layer 7 (通知 + 连接器，依赖 Layer 3-6)
  F44 NotificationChannel + ChannelAdapter (Telegram/企业微信/Slack/...) → F21
  F45 NotificationRule 订阅规则 → F44
  F46 通知管理 UI → F03, F44, F45
  F47 Issue Connector 标准接口 → F06
  F48 GitHub Issue Connector (内置) → F47, F34
  F49 Inbound Webhook 通用接收器 → F47
  F50 Connector 管理 UI → F03, F47
...
| F07 | ProjectRepo 多仓库 CRUD（含主仓库标记） | 2d | F04 | 6 |
| F08 | TicketRepoScope（工单绑定多 Repo） | 2d | F06, F07 | 6 |
| F09 | Workflow CRUD + Harness Git 存储 + 热重载 (fsnotify) | 5d | F04 | 6, 7 |
...
| F17 | Pickup/Finish 状态驱动 + assigned_agent_id 清空规则 | 2d | F05, F11 | 7 |
...
| F25 | Web UI 工单详情页（多 Repo PR 列表 + 活动日志 + Hook 历史） | 5d | F24 | 13 |
...
| ID | 任务 | 工作量 | 依赖 | PRD 章节 |
|----|------|--------|------|---------|
| F28 | **Setup Wizard**（4 步引导 + 模式选择 + DB 测试连接 + 主仓库 .openase/ 生成） | 5d | F04, F07, F10, F24 | 14 |
...
| F34 | GitHub Webhook 接收 + 签名验证 | 3d | F08 | 12 |
| F35 | PR 状态同步到 TicketRepoScope（open/merged/closed/changes_requested） | 3d | F34 | 12, 16 |
| F36 | 全部 PR merged → 触发 finish / PR closed → 触发 on_error 重试 | 2d | F35, F17 | 7, 12 |
...
**Layer 7 — 通知 + Issue Connector（第 11-14 周）**
...
| F47 | Issue Connector 标准接口（Connector interface + ConnectorSyncer） | 3d | F06 | 28 |
| F48 | GitHub Issue Connector（Pull + Push 双模 + 双向状态同步） | 4d | F47, F34 | 28 |
| F49 | Inbound Webhook 通用接收器 | 2d | F47 | 28 |
| F50 | Connector 管理 UI | 2d | F03, F47 | 28 |
...
| F63 | 工单绑定机器 + required_machine_labels 自动匹配 | 2d | F59, F11 | 25 |
...
| F78 | GitLab Issue Connector | 3d | F47 | 28 |
...
**Phase 2 里程碑（Week 14）：** 加上 F33-F50，GitHub 集成 + Agent 自治 + 角色库 + Dispatcher + 通知 + Issue Connector 全部可用。
