...
| `pr.opened` | 与工单关联的某个 Repo PR 已创建/打开 | ticket 必填 | `repo_id` 或 `repo_name`；可选：`pull_request_url`、`pull_request_number` |
| `pr.merged` | 与工单关联的某个 Repo PR 已合并 | ticket 必填 | `repo_id` 或 `repo_name`；可选：`pull_request_url`、`pull_request_number` |
| `pr.closed` | 与工单关联的某个 Repo PR 被关闭但未合并，或关闭事件需要被记录 | ticket 必填 | `repo_id` 或 `repo_name`；可选：`pull_request_url`、`pull_request_number`、`reason` |
...
| `pr.opened` | Activity-derived | PR 已创建/打开 | "📝 {{ ticket.identifier }} 已创建 PR：{{ pull_request_url }}" |
| `pr.merged` | Activity-derived | PR 已合并 | "🎉 {{ ticket.identifier }} 的 PR 已合并：{{ pull_request_url }}" |
| `pr.closed` | Activity-derived | PR 被关闭 | "📪 {{ ticket.identifier }} 的 PR 已关闭：{{ pull_request_url }}" |
