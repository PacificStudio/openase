...
**JSON 字段使用原则**：内容已知且会被查询过滤的字段用结构化列（如 `max_concurrent`、`auto_assign_workflow`）或 PostgreSQL 原生数组 `TEXT[]`（如 `labels`）。只有真正动态、形状不确定、不被查询的数据才用 JSONB（如 `Ticket.metadata`、`Workflow.hooks`、`AgentProvider.auth_config`、`ScheduledJob.ticket_template`、`ActivityEvent.metadata`）。
...
- 任何一个 Repo 的 PR 被 close without merge → 触发 on_error Hook，清空 `current_run_id`，指数退避后重试（Agent 可重新创建 PR）
...
3. **结束**：AgentRun 完成 → 把工单移到 `workflow.finish`；AgentRun 出错 → 指数退避后自动重试（不存在 "failed" 终态）
...
func (s *Scheduler) canDispatch(ctx context.Context, t *ticket.Ticket, wf *workflow.Workflow) bool {
    ...
    // pickup status 所属 Stage 并发已满 → 跳过
    if wf.PickupStageMaxActiveRuns > 0 &&
       s.pool.ActiveCountForStage(wf.PickupStageID) >= wf.PickupStageMaxActiveRuns {
        return false
    }
    // 并发数已满 → 跳过
...
4. 检查分发门槛：
   ...
   - pickup status 所属 Stage 若配置了 `max_active_runs`，则该 Stage semaphore 未满
   - per-workflow 并发未满
...
3. **Stage Semaphore**
   - 每个 TicketStage 可选配置 `max_active_runs`
   - 表示处于该 Stage 内、且已被编排引擎领取的 Ticket 同时最多允许多少个
   - 多个 Workflow 如果 pickup 的状态都属于同一个 Stage，则共享这一个 Stage semaphore
   - 典型例子：`In Progress` Stage 设为 1，则这一阶段任意时刻只允许 1 个 AgentRun 在干活
...
- `能不能跑` 取决于 global semaphore + provider semaphore + stage semaphore
...
| （重试/暂停） | 任一 PR closed without merge | 记录错误，清空 `current_run_id`，指数退避后重试（Agent 可以重新创建 PR） |
...
**没有硬编码状态机，但有可选的 Stage 分组。** Custom Status 仍然是唯一可流转的状态维度；Stage 只是对多个状态做 UI 分组和共享并发治理，不定义合法跳转规则。编排引擎通过 Workflow 的 `pickup / finish` 配置知道该关心哪些状态，并可额外受 pickup status 所属 Stage 的并发上限约束。
...
| id | UUID | 主键 |
| project_id | FK | 所属项目 |
| key | String | 稳定标识（如 `backlog`、`in_progress`、`done`），项目内唯一 |
| name | String | 阶段名称（如 `Backlog`、`In Progress`、`Review`） |
| position | Integer | 阶段排序位置 |
| max_active_runs | Integer (nullable) | 该 Stage 同时允许的最大活跃 AgentRun 数；为空表示不限制 |
...
| id | UUID | 主键 |
| project_id | FK | 所属项目 |
| stage_id | FK (nullable) | 所属 TicketStage；为空表示不参与阶段分组 |
...
编排引擎查询变为：`WHERE status_id = workflow.pickup_status_id AND current_run_id IS NULL AND retry_paused = false AND (next_retry_at IS NULL OR next_retry_at <= now())`。命中候选工单后，还需检查其 pickup status 所属 `TicketStage.max_active_runs` 是否已满。没有 fail 状态，没有硬编码状态机，Agent 出错就退避重试。
...
| Stage | 包含的默认状态 | `max_active_runs` 默认值 |
|------|---------------|-------------------------|
| Backlog | Backlog / Todo | null |
| In Progress | In Progress | null（团队可改为 `1`） |
| Review | In Review | null |
| Done | Done / Cancelled | null |

如果团队希望“任意时刻只能有 1 个 Agent 在做事”，可以把 `In Progress` Stage 的 `max_active_runs` 设为 `1`。这样所有 pickup 到该 Stage 下状态的 Workflow 都会共享这一限制，其他工单即使到了 pickup 列，也会等待前一个 AgentRun 结束后再被领取。
...
Stage 在看板上以分组头或分隔线呈现，用来表达“这些状态属于同一个阶段，并共享同一阶段容量”。例如 `In Progress` Stage 右上角可显示 `1 / 1`，表示该阶段并发已满。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/projects/:projectId/stages` | 列出项目的所有 TicketStage |
| POST | `/api/v1/projects/:projectId/stages` | 创建 TicketStage `{key, name, position, max_active_runs}` |
| PATCH | `/api/v1/stages/:stageId` | 更新 TicketStage（名称、排序、并发上限） |
| DELETE | `/api/v1/stages/:stageId` | 删除 TicketStage（其下状态变为未分组） |
| GET | `/api/v1/projects/:projectId/statuses` | 列出项目的所有 Custom Status（按 Stage 分组） |
| POST | `/api/v1/projects/:projectId/statuses` | 创建 Custom Status `{name, stage_id?, color, icon, position}` |
| PATCH | `/api/v1/statuses/:statusId` | 更新 Custom Status（名称、颜色、图标、排序） |
| DELETE | `/api/v1/statuses/:statusId` | 删除 Custom Status（该状态下的工单迁移到同 Stage 的默认状态或项目默认状态） |
...
-- 状态按 stage 分组渲染看板 / 统计阶段容量
CREATE INDEX idx_statuses_stage ON ticket_statuses (project_id, stage_id, position);
...
| 章节 | 影响 |
|------|------|
| 第六章 领域模型 | Ticket.status 改为 status_id FK；新增 TicketStage / TicketStatus；Workflow 新增 pickup_status_id / finish_status_id |
| 第七章 已重写 | 不再有硬编码状态机；编排引擎看 Workflow 的 pickup_status_id，并可受 Stage 并发限制约束 |
| 第八章 Hook | 触发点不再引用具体状态名，只关心编排生命周期事件 |
| 第十章 编排引擎 | 调度查询改为 `WHERE status_id = workflow.pickup_status_id AND current_run_id IS NULL`，并额外检查 Stage semaphore |
...
核心简化：**没有硬编码状态机。** 编排引擎只匹配 `status_id == pickup_status_id`，不做任何状态转换合法性校验；Stage 只负责 UI 分组和共享并发上限，不参与跳转合法性判断。状态流转完全由用户（看板拖拽）和 Agent（完成时移到 finish 状态）驱动。出错不改状态，只是退避重试。
...
4. **确认目标 Workflow 已绑定正确的 Agent**：例如 GPU 训练类 Workflow 应绑定到 `gpu-*` 机器上的 Provider
...
修复方案：Harness 中 `status.finish` 可以设为空。当 finish 为空时，编排引擎不自动移状态——由 Agent 自己通过 `openase ticket update --status "Success"` 或 `--status "Fail"` 决定。编排引擎只负责清空 `current_run_id`。
