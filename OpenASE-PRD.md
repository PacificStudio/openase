...
- `on_complete` Hook 聚合所有 Repo 的 CI 状态——只有全部 passing 才允许 Agent/人类执行后续显式状态推进（如移到 `in_review`）
...
- 典型事件包括：
  ...
  - agent claimed / runtime launching / ready / failed / completed
  - hook started / passed / failed
...
describe("createTicketStream", () => {
  it("updates store on ticket.created event", () => {
    ...
    // 模拟 SSE 事件
    const handler = mockES.addEventListener.mock.calls.find(
      (c) => c[0] === "ticket.created",
    )[1];
    handler({
      data: JSON.stringify({
        ticketId: "ASE-1",
        ticket: { id: "ASE-1", status: "backlog" },
      }),
    });
...
- 外层顺序先按 `TicketStage.position` 渲染 Stage 分组
- 每个 Stage 分组内再按 `TicketStatus.position` 渲染具体列
- `stage_id = null` 的状态不混入已分组 Stage，而是统一作为最终的 `Ungrouped` 分组显示在所有 Stage 分组之后
- `/api/v1/projects/:projectId/statuses` 返回的 `stage_groups` 就是看板的主渲染契约；前端不应再只根据平铺 `statuses` 自行推断组顺序
...
| GET    | `/api/v1/projects/:projectId/statuses`         | 列出项目的所有 Custom Status（按 Stage 分组；`stage_groups` 顺序即看板分组顺序，未分组状态位于最后一个 `Ungrouped` 分组） |
  ...
  "filters": [
    {
      "name": "default",
      "args": "value",
      "description": "变量为空时使用默认值",
      "example": "{{ ticket.description | default(\"无描述\") }}"
    },
...
- Provider 控制面响应必须显式暴露 `capabilities.ephemeral_chat`，至少包含：
  - `state`: `available` / `unavailable` / `unsupported`
  - `reason`: 当 provider 不能用于 Ephemeral Chat 时给出结构化原因；对已支持但暂不可用的 provider，优先复用 availability reason
