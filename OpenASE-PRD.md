...
2. 工单带有 Workflow 类型标签（用户自定义 string label，如 `Product Manager` / `QA` / `Backend Engineer`），平台再基于 Role / 状态语义 / Harness 信号导出稳定的 Workflow Family（如 `planning` / `test` / `coding`），用于推荐、分析与视觉语义
...
### 4.2 多 Workflow 标签与 Family

`workflow.type` 面向用户，是项目语言中的原始标签；平台内部再派生稳定的 `workflow family`，用于调度分析、HR Advisor、Dashboard 聚合和视觉语义。

| Workflow Family | 用户可见标签示例 | 职责 | 典型产出 |
|--------------|----------------|------|---------|
| planning | Product Manager / PRD / 需求分析 | 需求定义、范围澄清、规划拆解 | 可执行工单、优先级与范围结论 |
| dispatcher | Dispatcher / Triage / Routing | 工单分派、lane 路由、状态编排 | 状态变更、派发结果 |
| coding | Fullstack Developer / Backend Engineer / Frontend Engineer | 需求实现、功能开发、Bug 修复、重构 | Git Branch + PR + CI 绿灯 |
| review | Code Reviewer / Approval | 代码审查、审批、合并前把关 | Review 结论、阻塞点 |
| test | QA Engineer / Verification | 根据需求/代码编写测试，防回归 | 测试代码 + 覆盖率报告 |
| docs | Technical Writer / Documentation | 扫描代码变更，确保文档对齐最新实现 | 文档更新 PR |
| deploy | Release Captain / DevOps Engineer | 连接远程机器，更新配置，部署新版本 | 部署日志 + 版本验证结果 |
| security | Security Engineer / Audit | 代码安全分析，漏洞检测，编写 PoC | 安全报告 + 修复 PR |
| harness | Harness Optimizer / Prompt Tuning | 自动优化 Workflow Harness 内容（元工作流） | 更新后的 Harness 模板 |
| environment | Environment Provisioner / Bootstrap | 环境修复、机器准备、依赖恢复 | 可调度环境、修复记录 |
| research | Research Ideation / Experiment Runner | 调研、实验设计、假设验证 | 调研结论、实验结果 |
| reporting | Report Writer / Writeup | 报告整理、结论沉淀、对外写作 | 报告、总结、paper |
...
| 数据库 | PostgreSQL 16 | JSON 字段 + 全文搜索 + 成熟可靠；用户自行部署，terminal setup 负责引导连接 |
...
| 认证 | 双模（`disabled` / `oidc`） | 本地默认可关闭浏览器登录；需要人类浏览器登录时使用标准 OIDC |
...
- `internal/setup`、`internal/webui`：当前 terminal setup、legacy web bootstrap 与 Web UI 入口
...
| agent_run_summary_prompt | Text (nullable) | 项目级 terminal run summary prompt 覆盖；留空表示使用平台内置默认 prompt |
...
| type | String | 用户定义的 Workflow 类型标签；非空、保留大小写，推荐 `1..64` |
...
- `workflow.type` 是面向用户的原始标签，不再是硬编码枚举；平台在读取时会派生稳定的 `workflow family` 供分析、推荐、可视化和兼容逻辑使用。
- 兼容旧数据时，历史值如 `coding` / `test` / `doc` / `security` / `deploy` / `refine-harness` / `custom` 仍可作为合法标签读取，并通过分类器映射到对应 family。
...
| completion_summary_status | String (nullable) | terminal run summary 生成状态：`pending` / `completed` / `failed` |
| completion_summary_markdown | Text (nullable) | terminal run 的人类可读 Markdown 摘要 |
| completion_summary_json | JSON | summary 结果元数据（provider / model 等） |
| completion_summary_input | JSON | terminal 时刻冻结的结构化 summary 输入（步骤、命令、审批、输出摘录、文件快照等） |
| completion_summary_generated_at | DateTime (nullable) | summary 成功生成时间 |
| completion_summary_error | Text (nullable) | summary 失败原因，便于排障 |
...
- 当 `AgentRun.status` 进入 `completed` / `errored` / `terminated` 时，平台必须异步冻结本次运行的结构化 summary 输入与最终文件变更快照，并把 `completion_summary_status` 置为 `pending`；该过程不能阻塞原始 runtime 的 terminal path。
- terminal summary 是平台自有的 best-effort 后处理，不改变原始 `AgentRun.status` / Ticket 业务状态；失败时只更新 `completion_summary_status=failed` 与 `completion_summary_error`。
- terminal summary 默认使用与原始 run 相同的 Provider 类别生成；若该 provider 在运行结束后不可用，原始 run 保持终态不变，summary 进入失败态。
...
### 6.11.2 ProjectUpdateThread / ProjectUpdateComment（项目进展更新）

项目需要一个高信噪比的人类进展面板，用于表达“当前项目状态如何、为什么这样判断、下一步看什么”。这条线必须是**独立的一等资源**，不能把正文和评论塞进 `ActivityEvent.metadata`。
...
| deleted_at | DateTime | 删除时间 |
| deleted_by | String | 删除操作者 |
| last_activity_at | DateTime | 该 thread 的最近活动时间；thread edit、status change、comment append 都必须推进它 |
| comment_count | Integer | 当前评论计数；读取优化字段 |
...
|------|------|------|
| id | UUID | 主键 |
| thread_id | FK | 所属 ProjectUpdateThread |
| body_markdown | Text | 当前生效的 Markdown 正文 |
| created_by | String | 创建者标识 |
| created_at | DateTime | 创建时间 |
| updated_at | DateTime | 最近更新时间 |
...
- `ProjectUpdateThreadRevision` 保存 thread 正文与状态的历史版本；创建时必须写入 `revision_number=1`。
- `ProjectUpdateCommentRevision` 保存 comment 正文历史；创建时必须写入 `revision_number=1`。
- 每次 thread/comment 正文保存都必须追加 revision，禁止原地覆盖历史。
- 删除采用软删除；默认 UI 保留时间线位置并显示 deleted 占位，不能直接把记录硬移除。
- 项目级 `Updates` 页面必须按 `last_activity_at DESC` 排序，让最近被讨论的进展上浮。
- `Updates` 是“人工整理后的项目进展真相源”；`ActivityEvent` 只允许追加摘要性旁路事件，不能作为 thread/comment 正文存储。
- SSE 刷新可以复用项目 activity stream，但并发观察者必须在无手动刷新前提下看到新的 update / comment / delete / status change。

### 6.11.3 ActivityEvent（业务活动事件）
...
| `project_update_thread.created` | 项目进展 thread 创建 | project 必填 | `thread_id`、`status`；可选：`thread_title`、`created_by` |
| `project_update_thread.edited` | 项目进展 thread 正文被编辑 | project 必填 | `thread_id`；可选：`thread_title`、`edited_by` |
| `project_update_thread.deleted` | 项目进展 thread 被软删除 | project 必填 | `thread_id`；可选：`thread_title`、`deleted_by` |
| `project_update_thread.status_changed` | 项目进展 thread 的风险状态变化 | project 必填 | `thread_id`、`from_status`、`to_status`；可选：`thread_title`、`edited_by` |
| `project_update_comment.created` | 项目进展评论创建 | project 必填 | `thread_id`、`comment_id`；可选：`created_by` |
| `project_update_comment.edited` | 项目进展评论被编辑 | project 必填 | `thread_id`、`comment_id`；可选：`edited_by` |
| `project_update_comment.deleted` | 项目进展评论被软删除 | project 必填 | `thread_id`、`comment_id`；可选：`deleted_by` |
...
登录 / 首次进入
  ...
  → 选择组织
  → 选择项目
  → 在项目内通过侧边栏进入 Board / Updates / Workflows / Agents / Settings
  → 在 Board 中查看与拖拽工单
...
| 项目层 | 看板、进展更新、Workflow、Agent、审批、活动、成本、设置 | Project Dashboard、Board、Updates、Agents、Settings |
...
一个工具再强大，如果新用户在第一个小时内无法体验到价值，就会被放弃。OpenASE 需要把 **本地 runtime bootstrap** 和 **更广义的产品 onboarding** 分开描述：当前已落地的是 terminal-first local setup；项目空态引导、团队协作引导和企业治理引导仍属于后续产品体验设计，不能与当前 `openase setup` 混写。

### 14.1 当前默认路径：Terminal-first Local Setup

当前本地启动的默认入口是显式运行 `openase setup`；`openase up` 只是兼容入口，在首次运行且找不到配置时会委托到同一套 terminal setup。
...
默认 setup 流程全程留在终端，不自动打开浏览器，也不要求用户先进入 Web UI。当前已落地的步骤是：

1. 选择数据库来源：自动拉起本地 Docker PostgreSQL，或手动填写已有 PostgreSQL 连接。
2. 在 setup 中验证数据库连通性并执行 schema 初始化。
3. 检查关键本地 CLI（如 `git`、`codex`、`claude`）是否可用。
4. 选择浏览器认证模式：`disabled` 或 `oidc`；若选 `oidc`，setup 直接采集 issuer、client ID/secret、redirect URL、scopes 等字段。
5. 选择 setup 结束后的运行方式：`config-only`，或在支持的 Linux 会话中安装 current-user `systemd --user` 服务。
6. 写入 `~/.openase/config.yaml`、`~/.openase/.env`，并创建 `~/.openase/logs/`、`~/.openase/workspaces/`。
7. 初始化默认本地 Organization / Project、ticket statuses 和已检测到的 provider seed data。

当前 setup **不再**要求或默认执行以下动作：

- 不把 Web Setup Wizard 当成主路径。
- 不自动打开浏览器或在完成后自动跳转到 `/setup` / Dashboard。
- 不在 setup 中采集 repo URL、默认分支、多人协作模式或“个人 / 团队 / 企业”模式卡片。
- 不在仓库根目录生成 repo-local `.openase/` scaffold。

### 14.2 兼容路径：Legacy Web Setup 与 `openase up`

代码里仍保留了一个 legacy browser troubleshooting 路径：

```bash
openase setup --web --host 127.0.0.1 --port 19836
```

这个入口会启动轻量 HTTP server、暴露 `/setup`，并尝试打开浏览器；但它是 **隐藏且 deprecated 的兼容路径**，只应用于排障或过渡，不应在 PRD 中继续描述成当前默认 onboarding。

同样，`openase up` 也需要按两段语义描述：

1. 如果还没有 `~/.openase/config.yaml`，它会转入 terminal setup。
2. 如果配置已存在，它只负责安装或刷新当前用户服务定义，不等同于重新进入一个多步骤 Setup Wizard。

因此，`/setup`、浏览器自动打开、完成后自动跳转等行为都属于兼容/过渡表面，而不是当前本地 setup 的核心合同。

### 14.3 当前服务管理合同：Config-only + Linux `systemd --user`

当前 setup 在运行方式上只承诺两个选项：

- `config-only`：setup 只写 `~/.openase/*`，之后由用户手动运行 `openase all-in-one --config ~/.openase/config.yaml`。
- `current-user systemd --user`：仅当机器是 Linux 且当前登录会话支持 `systemctl --user` 时，setup 才会在写完配置后直接安装并校验托管服务。
...
- `openase up/down/restart/logs` 是当前用户服务管理命令；不是浏览器引导流程。
- setup 内联安装服务时，当前主路径只描述 Linux `systemd --user`；若环境不支持，会明确回退为 `config-only`。
- 实现层虽然保留了用户服务抽象和 `launchd` 兼容后端，但 PRD 不应再把 macOS `launchd` 写成当前 setup 的对等默认选项；它只能作为兼容实现或后续扩展提及。

### 14.4 当前范围、兼容范围与非目标

为避免 setup/service onboarding 再次漂移，本章采用以下边界：

- 当前已落地：
  - terminal-first local setup
  - `disabled` / `oidc` 两种认证模式
  - `config-only` / Linux current-user `systemd --user` 两种 runtime 选择
  - `~/.openase/` 下的配置、日志、workspace 与默认控制面数据初始化
- 兼容路径：
  - `openase setup --web` legacy browser flow
  - `openase up` 在“无配置时进入 setup / 有配置时安装服务”的双态行为
  - 非 Linux 平台上的用户服务抽象实现
- 非目标 / 未落地承诺：
  - 以 Web Setup Wizard 作为当前主路径
  - setup 完成后自动浏览器跳转
  - setup 时进行 repo 绑定、项目模板选择或 persona 分流
  - 把更广义的团队/企业 onboarding 混入本地 runtime bootstrap

以下 14.5+ 描述的是 OpenASE 已启动后的产品级 onboarding 设想，用来定义后续 UI/空态体验；它们不等同于当前 `openase setup` 已经提供的行为。

### 14.5 空 Project 的首次导览体验（产品级后续引导，不等同于当前 local setup）

空 Project onboarding 是 OpenASE 已启动后的**第二段产品引导**。当前 `openase setup` 负责把平台跑起来；空 Project 导览负责把一个刚创建好的项目真正推进到“可以让 Agent 开始工作”的状态。
...
- 用户完成 terminal setup 并启动 OpenASE 后，首次进入项目级 Dashboard
- 左侧边栏自动聚焦刚创建的项目
...
- 首次预置问题建议：
  ...
  - `我下一步应该先做什么？`
- 用户确认后，Project AI 可以通过 `platform_command_proposal` 帮助创建后续 Ticket；迁移期间仍兼容 legacy `action_proposal`
...
| 配置变更 | terminal setup / 手动编辑配置 → restart 托管服务 | 服务重启，前端 SSE 自动重连 |
...
| **全局配置** | 文件系统 | `~/.openase/config.yaml` | `openase setup` / 手动编辑 | serve + orchestrate (启动时读取) |
| **敏感配置** | 文件系统 | `~/.openase/.env` | `openase setup` / 手动编辑 | serve + orchestrate (启动时读取) |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/projects/:projectId/updates` | 列出项目进展 threads，按 `last_activity_at DESC` 排序 |
| POST | `/api/v1/projects/:projectId/updates` | 创建项目进展 thread |
| PATCH | `/api/v1/projects/:projectId/updates/:threadId` | 编辑项目进展 thread（含状态变更） |
| DELETE | `/api/v1/projects/:projectId/updates/:threadId` | 软删除项目进展 thread |
| GET | `/api/v1/projects/:projectId/updates/:threadId/revisions` | 读取 thread 历史版本 |
| POST | `/api/v1/projects/:projectId/updates/:threadId/comments` | 创建项目进展评论 |
| PATCH | `/api/v1/projects/:projectId/updates/:threadId/comments/:commentId` | 编辑项目进展评论 |
| DELETE | `/api/v1/projects/:projectId/updates/:threadId/comments/:commentId` | 软删除项目进展评论 |
| GET | `/api/v1/projects/:projectId/updates/:threadId/comments/:commentId/revisions` | 读取评论历史版本 |
...
- 如果升级后需要交互式补全配置，允许用户重新运行 `openase setup --force`；但新增配置项应优先提供安全默认值，而不是依赖 Web Setup Wizard 提示
...
### 25.10 产品级 Onboarding / Web UI 中的机器管理

当 OpenASE 已启动并进入 Web UI 后，可在设置中提供机器管理页面（设置 → 机器）：
...
- 现有工单与 Workflow 的 family 分布（如 `coding` / `test` / `docs` / `deploy` / `planning`）
- 工单状态分布与各状态的排队压力（例如 `Backlog`、`待测试`、`待文档`）
- 已有角色/Workflow 列表，以及每个 Workflow 的原始 type label、派生 family、`pickup` / `finish` 状态绑定
...
- 每条推荐同时给出建议的用户可见 Workflow type label 与内部 Workflow family
...
- HR Advisor 不允许直接依赖 `workflow.type == "coding"` 这一类原始字符串相等判断；必须通过显式规则分类器，从 role slug、type label alias、workflow name、状态语义、技能与 Harness 信号派生 family。
...
| `workflow.type` | string | 用户可见 Workflow 类型标签（原始值） | `Backend Engineer` |
| `workflow.family` | string | 平台派生的稳定 Workflow Family | `coding` |
...
- `harness_editor`、`command_palette`、`cron_input` 等入口属于 **Ephemeral Chat**
  - 临时对话
  - 关闭即结束
  - transcript 默认不持久化
- `project_sidebar` 与 `ticket_detail(ticket-focused)` 属于 **Project Conversation**
  - 项目级持续对话
  - transcript 持久化
  - 允许关闭 UI 后续接之前的对话
...
当前实现阶段还需要明确 editor-side AI surface 的真实产品合同：

- `Project AI`（`project_sidebar`）仍然是 machine-aware 的 Direct Chat 主线，目标是 provider 跑在哪台 machine，就在那台 machine 上执行
- `Harness AI` 当前是 **local-only** 的 editor-side Ephemeral Chat
  - 只允许选择绑定在 OpenASE 服务宿主机本地的 Codex / Claude / Gemini provider
  - 远端 machine provider 在 UI 与 API 中都必须视为 unsupported，直到 editor-side machine-aware execution 真正落地
- `Skill AI` 当前不是通用 Ephemeral Chat，而是第三十四章的 skill refinement loop
  - Phase 1 合同是 **local Codex-only**
  - Claude / Gemini provider 与 remote Codex provider 都不在当前支持范围内

因此，不能把 `ephemeral_chat` 能力直接当作所有 editor-side AI surface 的统一真相源；`Harness AI` 与 `Skill AI` 必须各自暴露独立、可测试的 provider eligibility 合同。
...
|------|----------------|----------------------|
| 典型入口 | Harness AI、Cron AI | `project_sidebar`、`ticket_detail(ticket-focused)` |
...
| 审批 | 仅 `action_proposal` 的用户确认 | `platform_command_proposal`（迁移期兼容 `action_proposal`）+ Codex 原生工具审批 / 用户输入 interrupt |
...
- `ticket_detail(ticket-focused)`
  - 复用 Project Conversation
  - 注入工单完整 capsule：标题、描述、状态、优先级、attempt/retry、依赖、repo scope / PR、最近活动、Hook 历史、assigned agent / current run / target machine 摘要
  - runtime env 追加 `OPENASE_TICKET_ID`
...
- `project_sidebar` / ticket-focused Project Conversation 的平台写操作优先通过 `platform_command_proposal`；其余 direct chat 入口仍可沿用 `action_proposal`
- 迁移期间后端必须兼容 legacy `action_proposal`，直到 Project Conversation 完成切换
...
- **平台写操作确认**
  - `Project Conversation` 优先通过 `platform_command_proposal` 提案；其它入口或迁移期 fallback 仍可使用 `action_proposal`
  - 用户确认后，平台 API 才真正执行
- **CLI 原生工具审批 / 用户输入**
...
### 31.4.1 平台写操作：Project Conversation 优先 `platform_command_proposal`

当用户在 `Project Conversation` 中要求创建 / 修改工单、添加项目进展、调整 Workflow、更新项目配置等平台写操作时，AI 必须优先输出结构化 `platform_command_proposal`，等待用户确认。该协议必须是受限命令 DSL，而不是原始 REST path + body。
...
1. 解析 proposal JSON 为 typed domain command
2. 将 `project` / `ticket` / `status` / `workflow` 等人类可读引用解析为 UUID
3. 调用既有 service-layer mutation 执行，而不是回放手写内部 HTTP 请求
4. 将逐条结构化执行结果回写到 conversation transcript

执行流程：

1. AI 输出 `platform_command_proposal`
2. 前端渲染确认 UI
3. 用户点击确认
4. 由后端解析命令、解析引用、执行既有 service mutation，并把结构化结果回写为 conversation entries
...
- 迁移期间必须继续接受 legacy `action_proposal`
- 新 prompt / UI / executor 默认使用 `platform_command_proposal`
- 当引用无法唯一解析时，AI 应优先追问澄清，而不是猜测 UUID 或 REST 字段
...
- 平台写操作仍然必须走 `platform_command_proposal` 或迁移期兼容的 `action_proposal`，不能借由 Codex 工具审批直接放行平台变更
...
| 工单详情页底部 | Project Conversation（ticket-focused） | 工单完整 capsule + `OPENASE_TICKET_ID` + 持久 conversation workspace | “为什么失败了”、“帮我拆成子工单”、“Agent 做了什么” |
...
所有入口都需要带出与自身真实运行合同一致的 provider selector：

- `Project AI` 继续允许在 Claude Code / Codex / Gemini CLI 之间切换当前对话使用的 provider
- `Harness AI` 只显示当前 surface 真正可执行的本地 provider
- `Skill AI` 只显示第三十四章 skill refinement 当前真正支持的本地 Codex provider

补充交互约束：

- `project_sidebar` 采用常规聊天体验，不再使用固定 turn cap 作为 transcript UX
- `ticket_detail` 不再走独立的 ephemeral `Ticket AI`；它必须打开或复用 Project AI 的 ticket-focused conversation，并保持 ticket-native inline affordance
- `project_sidebar` 可在当前 UI 会话中打开多个 conversation tabs；页面内的 Project AI live transport 必须收敛为按 `project_id` 建立的单条 multiplexed SSE，再按 `conversation_id` 分发到各 tab，而不是为每个 tab 单独建 stream
...
| `chat_entries` | `id`, `conversation_id`, `turn_id`, `seq`, `kind`, `payload_json` | 追加式 transcript，覆盖 text / diff / platform_command_proposal / action_proposal / interrupt / result |
...
- `project_sidebar` 可恢复同一项目下的多个 persisted conversation tabs，但单个页面内无论恢复多少个 tabs，都只允许保留 1 条 Project AI multiplexed SSE
- active tab 在 restore 后应立刻 hydrate transcript；inactive tabs 在恢复期间可只补 runtime 状态、最新摘要、未读标记和 `needsHydration`，等用户切回该 tab 时再做 transcript reconcile
...
event: message
data: {"type":"platform_command_proposal","summary":"创建 3 个子工单","commands":[...]}
...
#### 31.8.4 Proposal 执行的审计语义

Project Conversation 中的平台写操作默认通过 `platform_command_proposal` + 人类确认完成；迁移期间仍兼容 `action_proposal`。最终审计语义必须显式区分发起源：
...
- 无论是直接调用 service-layer mutation 还是迁移期 replay internal API，都必须把上述 audited origin 显式写入 mutation payload 或等价审计字段
...
| 平台操作 | `ticket_agent` principal token 直接执行 | 默认 `platform_command_proposal` 经用户确认后执行；迁移期兼容 `action_proposal`；token 身份为 `project_conversation` principal |
...
```
用户下载 openase 二进制 → ./openase setup（或首次运行 ./openase up）
→ 在终端完成 PostgreSQL / 认证模式 / runtime 选择
→ setup 检测到 Claude Code 已安装并写入 `~/.openase/*`
→ 启动本地 OpenASE 并进入默认项目
...
Layer 4 (Onboarding，依赖 Layer 2-3)
  F28 terminal-first local setup（数据库、认证、runtime bootstrap） → F04, F10
  F29 current-user 服务管理（`systemd --user` 主路径，`launchd` 兼容） → F01
  F30 openase up 启动流程（无配置进入 setup；有配置更新服务） → F28, F29
...
| ID | 任务 | 工作量 | 依赖 | PRD 章节 |
|----|------|--------|------|---------|
| F28 | **terminal-first local setup**（数据库准备/校验 + CLI 检查 + auth/runtime 选择 + 默认控制面数据初始化；保留 legacy web 兼容入口） | 5d | F04, F10 | 14 |
| F29 | current-user 服务管理（Linux `systemd --user` 主路径 + `openase up/down/restart/logs`；`launchd` 作为兼容实现） | 3d | F01 | 14 |
| F30 | `openase up` 启动流程（检测配置 → 无配置进入 setup / 有配置更新服务） | 2d | F28, F29 | 14 |
...
| F76 | OIDC 认证（Provider 实现 + terminal setup prompts） | 4d | F04 | 5, 14 |
...
**Phase 1 里程碑（Week 8）：** F01-F32 全部完成，一个用户可以 `openase setup` / `openase up` → terminal setup → 创建工单 → Claude Code Agent 自动编码 → 看板实时更新 → 工单完成。
