...
const desktopVersion = process.env.OPENASE_DESKTOP_VERSION || packageJson.version
...
if (!skipBuild) {
  run('make', ['build-web', 'build', `VERSION=${desktopVersion}`, `OPENASE_BIN=${bundleBinaryPath}`], {
    cwd: repoRoot,
  })
}
